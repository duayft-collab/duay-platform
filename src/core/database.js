...
window._dbKullaniciFiltreUygula = function(liste) {
  ...
  var cu = window.CU?.();
  /* ALIS-005: fail-closed — auth ready değilse boş liste (user veri kaçağı engeli) */
  if (!cu) return [];
  ...
  /* ALIS-005: fail-closed — uid eksikse boş liste */
  if (!uid) return [];
...
/** @returns {Array<Object>} */
function loadTasks()       { var d = _read(KEYS.tasks); var arr = Array.isArray(d) ? d : DEFAULT_TASKS; var filtreli = arr.map(function(k) { return window._migrateRecord ? window._migrateRecord(k) : k; }).filter(function(k) { return !k.isDeleted; }); var cu = window.CU?.(); /* ALIS-005: fail-closed — cu yoksa boş */ if (!cu) return []; /* ALIS-004: yönetici rolleri bypass */ var _yoneticiRoller = ['super_admin','admin','manager','lead']; if (_yoneticiRoller.indexOf(cu.role) !== -1 || _yoneticiRoller.indexOf(cu.rol) !== -1) return filtreli; var uid = cu.uid || cu.id || ''; /* ALIS-005: fail-closed — uid yoksa boş */ if (!uid) return []; /* PUSULA-TASK-ISOLATION-001: sahipsiz task'ları non-admin'e gösterme (DEFAULT_TASKS sızıntısı fix) */
...
function loadCari(opts) {
  ...
  var cu = window.CU?.();
  /* ALIS-005: fail-closed — cu yoksa boş (kullaniciFiltered zaten boş ama net) */
  if (!cu) return [];
  /* ALIS-004: yönetici rolleri bypass — hiddenFrom post-filter atla */
  var _yoneticiRoller = ['super_admin','admin','manager','lead'];
  if (_yoneticiRoller.indexOf(cu.role) !== -1 || _yoneticiRoller.indexOf(cu.rol) !== -1) return kullaniciFiltered;
  var uid = String(cu.uid || cu.id || '');
  /* ALIS-005: fail-closed — uid yoksa boş */
  if (!uid) return [];
...
function startRealtimeSync() {
  ...
  const CRITICAL_COLS = ['kur','users','tasks','cari','activity','updateLog','trash','pusula','notifications','urunler','bankalar','announcements','satisTeklifleri','alisTeklifleri','teklifSartlar','ihracatDosyalar','crm','kargo','stok']; /* [SYNC-CRITICAL-EXPAND-001] cari zaten vardi; bankalar eklendi */  /* LS-SYNC-009-FIX-002: pusula + notifications CRITICAL'a eklendi (görevler eksikti) */
  ...
  const MODULE_COLLECTIONS = {
    ...
    'satis-teklifleri':     ['satisTeklifleri','urunler','teklifSartlar','cari'],
    'satis-rapor':          ['satisTeklifleri','urunler','cari'],
    'siparisler':           ['satisTeklifleri','kargo','navlun'],
    'finans':               ['odemeler','tahsilat','bankalar'],
    'odemeler':             ['odemeler','bankalar','cari'],
    'tahsilat':             ['tahsilat','bankalar','cari'],
    ...
    'cari-karsilastirma':   ['cari','odemeler','tahsilat'],
    'crm':                  ['crm','greetings','cari'],
    'announce':             ['announcements'],
    'evrak':                ['evrak','resmiEvrak','arsivBelgeler','tebligat','etkinlik','numune'],
    'arsiv-hub':            ['arsivBelgeler','resmiEvrak','evrak'],
  ...
  const SYNC_MAP = [
    ...
    /* V192b — TAHKİM: ihracat detay (V184a5+V191d evrak/kargo + manuel alanlar)
...
// [URUN-RENDER-FIX-001 START] Field schema bridge — duayName/urunAdi, image/gorsel, duayKodu/urunKodu, vendorName/tedarikci
// Idempotent: sadece eksik field doldurulur. Mevcut değerlere dokunulmaz.
window._normalizeUrunFields = function(u) {
  if (!u || typeof u !== 'object') return u;
  // ad — duayName ↔ urunAdi ↔ duayAdi
  if (u.duayName && !u.urunAdi)  u.urunAdi  = u.duayName;
  if (u.urunAdi  && !u.duayName) u.duayName = u.urunAdi;
  if (u.duayName && !u.duayAdi)  u.duayAdi  = u.duayName;
  if (u.duayAdi  && !u.duayName) u.duayName = u.duayAdi;
  // görsel — image ↔ gorsel ↔ imageUrl
  if (u.image    && !u.gorsel) u.gorsel = u.image;
  if (u.gorsel   && !u.image)  u.image  = u.gorsel;
  if (u.imageUrl && !u.gorsel) u.gorsel = u.imageUrl;
  // kod — duayKodu ↔ urunKodu
  if (u.duayKodu && !u.urunKodu) u.urunKodu = u.duayKodu;
  if (u.urunKodu && !u.duayKodu) u.duayKodu = u.urunKodu;
  // vendor — vendorName ↔ tedarikci
  if (u.vendorName && !u.tedarikci)  u.tedarikci  = u.vendorName;
  if (u.tedarikci  && !u.vendorName) u.vendorName = u.tedarikci;
  return u;
};
