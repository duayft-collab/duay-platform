...
function _odmToTRY(amount, currency, record) {
  if (record && (record.lockedRate || record.kurRate) && currency !== 'TRY') {
    return parseFloat(amount || 0) * (record.lockedRate || record.kurRate);
  }
...
function renderOdemeler() {
  ...
  const weekAmt = _myOdm.filter(o => !o.paid && o.due && o.due >= today && o.due <= weekEndStr)
                     .reduce((s,o) => s + _odmToTRY(parseFloat(o.amount)||0, o.currency||'TRY', o), 0);
  ...
  var _late60Amt = _late60.reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount) || 0, o.currency || 'TRY', o); }, 0);
  ...
  if (_summaryEl) {
    var _filteredNet = items.reduce(function(s,o) { var amt = _odmToTRY(parseFloat(o.amount)||0, o.currency||'TRY', o); return o._src === 'tahsilat' ? s + amt : s - amt; }, 0);
  ...
  Object.keys(_dayGroups).sort(function(a, b) { return b.localeCompare(a); }).forEach(function(dayKey) {
    ...
    var dayNet = dayItems.reduce(function(s, o) { var amt = _odmToTRY(parseFloat(o.amount)||0, o.currency||'TRY', o); return (o._src === 'tahsilat' || o.tip === 'tahsilat') ? s + amt : s - amt; }, 0);
    ...
    dayItems.forEach(function(o) {
      ...
      var rowHtml =
        ...
        + '<div style="flex:1;display:grid;grid-template-columns:1fr auto;gap:4px 12px;padding:8px 12px;min-width:0">'
          + '<div style="min-width:0">'
            + '<div style="font-size:12px;font-weight:500;color:var(--t);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + window._esc(o.cariName || o.name || '—') + (o.docs && o.docs.length ? ' <span style="font-size:10px;cursor:pointer" title="' + o.docs.length + ' belge" onclick="event.stopPropagation();' + (isTah ? 'viewTahDoc' : 'viewOdmDoc') + '(' + o.id + ',0)">📎</span>' : '') + '</div>'
            + '<div style="font-size:10px;color:var(--t3);margin-top:2px;display:flex;gap:4px;flex-wrap:wrap;align-items:center">'
              ...
              + (o.yontem ? '<span style="background:var(--s2);padding:1px 5px;border-radius:4px">' + window._esc(o.yontem) + '</span>' : '')
      ...
      var peekHtml =
        ...
        + '<div style="padding:10px 16px 10px 60px;background:var(--s2);display:grid;grid-template-columns:repeat(5,1fr);gap:8px;font-size:11px">'
          + '<div><div style="color:var(--t3);font-size:9px;text-transform:uppercase;margin-bottom:2px">Cari</div><div style="font-weight:500;color:var(--t)">' + window._esc(o.cariName || '—') + '</div></div>'
          + '<div><div style="color:var(--t3);font-size:9px;text-transform:uppercase;margin-bottom:2px">Fatura No</div><div style="font-weight:500;color:var(--t)">' + window._esc(o.docNo || o.ref || '—') + '</div></div>'
          + '<div><div style="color:var(--t3);font-size:9px;text-transform:uppercase;margin-bottom:2px">Job ID</div><div style="font-weight:500;color:var(--t)">' + window._esc(o.taskId || '—') + '</div></div>'
          + '<div><div style="color:var(--t3);font-size:9px;text-transform:uppercase;margin-bottom:2px">Kaydeden</div><div style="font-weight:500;color:var(--t)">' + window._esc(_creator?.name || '—') + ' · ' + _fmtTs(o.ts) + '</div></div>'
          + '<div><div style="color:var(--t3);font-size:9px;text-transform:uppercase;margin-bottom:2px">Ek Belge</div><div style="font-weight:500;color:var(--t)">' + (o.docs && o.docs.length ? o.docs.map(function(d, i) { return '<span onclick="event.stopPropagation();' + (isTah ? 'viewTahDoc' : 'viewOdmDoc') + '(' + o.id + ',' + i + ')" style="cursor:pointer;color:#185FA5;font-size:10px;display:block">📎 ' + window._esc(d.name || 'Belge ' + (i + 1)) + '</span>'; }).join('') : '<span style="color:var(--t3);font-size:10px">Belge yok</span>') + '</div></div>'
        + '</div>'
        + '<div style="padding:6px 16px 8px 60px;background:var(--s2);border-bottom:0.5px solid var(--b);display:flex;gap:6px;align-items:center">'
          + (function() {
            ...
            return (_canEdit ? '<button onclick="event.stopPropagation();' + (isTah ? 'openTahsilatModal(' + o.id + ')' : 'openOdmModal(' + o.id + ')') + '" class="odm-hdr-btn" style="font-size:10px;padding:4px 10px">Düzenle</button>' : '')
              ...
              + (isRejected && o.rejectionNote ? '<div style="font-size:9px;color:#dc2626;padding:2px 0">Ret sebebi: ' + window._esc(o.rejectionNote) + '</div>' : '')
...
function openOdmModal(id) {
  ...
  var cariOpts = '<option value="">— Cari Seçin * —</option>' + cariList.map(function(c) {
    ...
    var label = window._esc(c.name) + ' (' + (c.type || '') + ')' + (isPending ? ' ⏳ Onay Bekliyor' : '');
    return '<option value="' + window._esc(c.name) + '"' + (o?.cariName === c.name ? ' selected' : '') + (isPending ? ' disabled style="color:#999"' : '') + '>' + label + '</option>';
  }).join('');

  // Görev listesi
  var taskList = typeof loadTasks === 'function' ? loadTasks().filter(function(t) { return !t.done; }).slice(0, 50) : [];
  var taskOpts = '<option value="">— Görev Seçin —</option>' + taskList.map(function(t) { return '<option value="' + t.id + '"' + (o?.taskId == t.id ? ' selected' : '') + '>' + window._esc(t.title.slice(0, 40)) + '</option>'; }).join('');
  ...
  mo.innerHTML = '<div class="moc" style="width:560px;max-width:96vw;padding:0;overflow:hidden;border-radius:16px;max-height:92vh;display:flex;flex-direction:column">'
    ...
    + '<input class="fi" id="odm-f-name" placeholder="Örn: Ofis Kirası" style="font-size:13px;padding:10px 12px;border-radius:8px" value="' + window._esc(o?.name||'') + '"></div>'
    ...
    + '<input class="fi" id="odm-f-docno" placeholder="FTR-2026-001" style="font-size:13px;padding:10px 12px;border-radius:8px" value="' + window._esc(o?.docNo||'') + '"></div>'
    ...
    + '<textarea class="fi" id="odm-f-note" rows="2" style="resize:none;font-size:13px;padding:10px 12px;border-radius:8px" placeholder="Açıklama…">' + window._esc(o?o.note||'':'') + '</textarea></div>'
...
function saveOdm() {
  ...
  if (_selCari && _cariCreditLimit > 0) {
    ...
    var cariToplamBorc = cariOdm.reduce(function(sum, o) { return sum + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
...
function markOdmPaid(id) {
  ...
  setTimeout(() => {
    ...
    window.addNotif?.('📎', `"${window._esc(o.name)}" için dekont yüklenmedi`, 'warn', 'odemeler');
  }, 1500);
}
...
function postponeOdm(id) {
  ...
  mo.innerHTML = `<div class="moc" style="max-width:400px;padding:0;border-radius:12px;overflow:hidden">
    <div style="padding:14px 20px;border-bottom:1px solid var(--b)">
      ...
      <div style="font-size:12px;color:var(--t3);margin-top:2px">${window._esc(o.name)} — ${o.due || '—'}</div>
    </div>
...
function _savePostpone(id) {
  ...
  window.addNotif?.('⏳', '"' + window._esc(o.name) + '" erteleme onayı bekliyor — ' + window._esc(reason), 'warn', 'odemeler');
...
function _odmImportRenderPreview(fileName, parsed, errors) {
  ...
  previewRows.forEach(function(r, i) {
    ...
    html += '<div style="display:grid;grid-template-columns:30px 1fr 90px 60px 80px 90px 1fr 90px;gap:0;padding:6px 10px;border-bottom:1px solid var(--b);font-size:11px;' + bg + '">'
      ...
      + '<div style="font-weight:600;color:' + (hasError && !r.name ? '#EF4444' : 'var(--t)') + '">' + (r.name ? window._esc(r.name) : '<i style="color:#EF4444">eksik</i>') + '</div>'
      ...
      + '<div style="color:var(--t3);font-size:10px">' + (r.cari ? window._esc(r.cari) : '—') + '</div>'
      + '<div style="color:var(--t3);font-size:10px;font-family:monospace">' + (r.docNo ? window._esc(r.docNo) : '—') + '</div>'
    + '</div>';
...
window._odmImportConfirm = function() {
  ...
  _odmImportRows.forEach(function(r) {
    ...
    var entry = {
      ...
      amountTRY:   _odmToTRY(r.amount || 0, r.currency || 'TRY', r),
      kurMode:     _odmRatesMode,
...
function openTahsilatModal(id) {
  ...
  var tahCariOpts = '<option value="">— Cari Seçin * —</option>' + tahCariList.map(function(c) {
    ...
    var lbl = window._esc(c.name) + ' (' + (c.type || '') + ')' + (isPend ? ' ⏳' : '');
    return '<option value="' + window._esc(c.name) + '"' + (o?.cariName === c.name ? ' selected' : '') + (isPend ? ' disabled style="color:#999"' : '') + '>' + lbl + '</option>';
  }).join('');
  ...
  html += '<input class="fi" id="tah-f-name" placeholder="Örn: Mart ayı satış tahsilatı" style="font-size:13px;padding:10px 12px;border-radius:8px" value="' + window._esc(o?.name||'') + '"></div>';
  ...
  html += '<input class="fi" id="tah-f-ref" placeholder="INV-2026-001" style="font-size:13px;padding:10px 12px;border-radius:8px" value="' + window._esc(o?.ref||'') + '"></div>';
  ...
  html += '<textarea class="fi" id="tah-f-note" rows="2" style="resize:none;font-size:13px;padding:10px 12px;border-radius:8px" placeholder="Ek bilgiler...">' + window._esc(o?o.note||'':'') + '</textarea></div>';

  // Hidden alanlar
  html += '<input type="hidden" id="tah-f-eid" value="' + (o?o.id:'') + '">';
  html += '<input type="hidden" id="tah-f-actual" value="' + (o?.actualDate||'') + '">';
  html += '<input type="hidden" id="tah-f-banka" value="' + window._esc(o?.banka||'') + '">';
...
function saveTahsilat() {
  ...
  if (!eid && !_isAdminO()) {
    ...
    _yoneticiler.forEach(function(m) {
      window.addNotif?.('💰', 'Yeni tahsilat onay bekliyor: ' + window._esc(name) + ' - ' + amount + ' ' + currency + ' (' + (_CUo()?.name || '') + ')', 'warn', 'odemeler', m.id);
    });
...
function exportOdmPaymentPDF(id) {
  ...
  const html = '<html><body style="font-family:sans-serif;padding:40px;max-width:600px;margin:0 auto">'
    ...
    + '<tr><td style="padding:10px;font-weight:600">IBAN</td><td style="padding:10px">' + window._esc(o.iban || 'Belirtilmemiş') + '</td></tr>'
    + '<tr style="background:#f5f5ff"><td style="padding:10px;font-weight:600">Not</td><td style="padding:10px">' + window._esc(o.note || '—') + '</td></tr>'
    + '</table>'
...
function approveOdm(id) {
  ...
  if (o.approvalStatus === 'pending_postpone' && o.postponeDate) {
    ...
    window.addNotif?.('✅', '"' + window._esc(o.name) + '" erteleme onaylandı → ' + o.due, 'ok', 'odemeler');
  } else {
    window.toast?.('Ödeme onaylandı', 'ok');
  }
...
function openApprovalFlow(odmId) {
  ...
  mo.innerHTML = '<div class="moc" style="max-width:420px;padding:0;border-radius:14px;overflow:hidden">'
    + '<div style="padding:16px 20px;border-bottom:1px solid var(--b)">'
      ...
      + '<div style="font-size:11px;color:var(--t3);margin-top:2px">' + window._esc(o.name) + ' — ' + amount.toLocaleString('tr-TR') + ' TL</div>'
    ...
    + '<div style="padding:16px 20px">'
      + '<div class="fg"><div class="fl">ARA ONAYCI <span style="color:var(--rd)">*</span></div>'
        + '<select class="fi" id="oaf-approver">' + managers.map(u => '<option value="' + u.id + '">' + window._esc(u.name) + ' (' + u.role + ')</option>').join('') + '</select></div>'
...
window._submitApprovalFlow = function(odmId) {
  ...
  window.addNotif?.('💰', '"' + window._esc(o.name) + '" onayinizi bekliyor', 'warn', 'odemeler', approverId);
  ...
  mo.innerHTML = '<div class="moc" style="max-width:420px;padding:0;border-radius:12px;overflow:hidden">'
    + '<div style="padding:14px 20px;border-bottom:1px solid var(--b);display:flex;align-items:center;justify-content:space-between">'
      + '<div><div style="font-size:14px;font-weight:700;color:var(--t)">Onay Gecmisi</div>'
        + '<div style="font-size:11px;color:var(--t3)">' + window._esc(o.name||'') + '</div></div>'
    ...
    + '<div style="padding:16px 20px;max-height:50vh;overflow-y:auto">'
      + (log.length ? log.map((l,i) => '<div style="display:flex;gap:10px;padding-bottom:12px;'+(i<log.length-1?'border-left:2px solid var(--b);margin-left:5px;padding-left:16px':'margin-left:5px;padding-left:16px')+'">'
        ...
        + '<div><div style="font-size:12px;font-weight:500;color:var(--t)">' + window._esc(l.actorName||'?') + ' — ' + window._esc(l.action||'') + '</div>'
          + '<div style="font-size:10px;color:var(--t3)">' + (l.ts||'').slice(0,16) + '</div>'
          + (l.note ? '<div style="font-size:11px;color:var(--t2);margin-top:2px">' + window._esc(l.note) + '</div>' : '')
...
window._odmTaskSearch = function(val) {
  ...
  _odmTaskTimer = setTimeout(function() {
    ...
    if (found) {
      badge.innerHTML = '<span style="color:#10B981;font-weight:600">&#10003; ' + window._esc(found.title) + '</span>';
...
function openBankaManager() {
  ...
  var listHTML = bankalar.length ? bankalar.map(function(b) {
    return '<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid var(--b)">'
      + '<div style="flex:1;min-width:0">'
        + '<div style="font-size:12px;font-weight:600;color:var(--t)">' + window._esc(b.name) + ' <span style="font-size:10px;color:var(--t3)">(' + (b.hesapTur || 'TRY') + ')</span></div>'
        + '<div style="font-size:10px;color:var(--t3);font-family:monospace">' + window._esc(b.iban || '—') + (b.swift ? ' · SWIFT: ' + window._esc(b.swift) : '') + '</div>'
      + '</div>'
  ...
  if (isBanka && !wrap.dataset.loaded) {
    ...
    if (sel && bankalar.length) {
      sel.innerHTML = '<option value="">— Banka Seçin * —</option>' + bankalar.map(function(b) {
        return '<option value="' + b.id + '">' + window._esc(b.name) + ' (' + (b.hesapTur || 'TRY') + ') — ' + window._esc((b.iban || '').slice(0, 16)) + '...</option>';
      }).join('');
    }
  }
}
...
function _bankaDropdownHTML(prefix, o) {
  ...
  return '<div id="' + prefix + '-banka-wrap" style="display:' + (isBanka ? 'block' : 'none') + ';margin-top:8px" data-loaded="' + (isBanka ? '1' : '') + '">'
    ...
    + bankalar.map(function(b) {
        return '<option value="' + b.id + '"' + (o?.bankaId == b.id ? ' selected' : '') + '>' + window._esc(b.name) + ' (' + (b.hesapTur || 'TRY') + ') — ' + window._esc((b.iban || '').slice(0, 16)) + '...</option>';
      }).join('')
    + '</select></div>'
    + '</div>';
}
...
function openBudgetManager() {
  ...
  Object.entries(ODM_CATS).forEach(([k, v]) => {
    ...
    const spent = all.filter(o => o.cat === k && (o.paidTs||o.due||'').startsWith(thisMonth))
                     .reduce((s, o) => s + (_odmToTRY(o.amount, o.currency||'TRY', o)), 0);
    ...
    try {
      ...
      lines.slice(1).forEach(line => {
        ...
        const found = all.find(o => {
          ...
          const amtMatch  = Math.abs(_odmToTRY(o.amount, o.currency||'TRY', o) - Math.abs(amount)) < 10;
...
function openFaizHesap(id) {
  ...
  const anapar = _odmToTRY(o.amount, o.currency||'TRY', o);
...
function openTedarikciKart(name) {
  ...
  const total  = items.reduce((s,o) => s + _odmToTRY(o.amount, o.currency||'TRY', o), 0);
  const paid   = items.filter(o => o.paid).reduce((s,o) => s + _odmToTRY(o.amount, o.currency||'TRY', o), 0);
...
function openCurrencyReport() {
  ...
  [...all, ...tah].forEach(o => {
    ...
    const tl = _odmToTRY(o.amount||0, o.currency||'TRY', o);
  ...
  Object.entries(curGroups).forEach(([c, d]) => {
    ...
    const tl = all.filter(o=>(o.currency||'TRY')===c).reduce((s,o)=>s+_odmToTRY(o.amount||0, c, o),0);
...
function openOdmKPIDashboard() {
  ...
  const paidAmt = paidThisMonth.reduce((s,o)=>s+_odmToTRY(o.amount||0, o.currency||'TRY', o),0);
  const tahAmt  = tah.filter(o=>o.collected&&(o.collectedTs||'').startsWith(thisMonth))
                     .reduce((s,o)=>s+_odmToTRY(o.amount||0, o.currency||'TRY', o),0);
  ...
  mo.innerHTML = '<div class="moc" style="max-width:580px;padding:0;border-radius:16px;overflow:hidden">'
    ...
    + (all.filter(o=>o.approvalStatus==='pending').length ? '<div style="padding:10px 12px;background:rgba(245,158,11,.08);border-left:3px solid #F59E0B;border-radius:0 8px 8px 0">'
      ...
      + all.filter(o=>o.approvalStatus==='pending').slice(0,3).map(o=>'<div style="font-size:11px;color:var(--t3);margin-top:3px">• '+o.name+' — ₺'+Math.round(_odmToTRY(o.amount||0, o.currency||'TRY', o)).toLocaleString('tr-TR')+'</div>').join('')
      + '</div>' : '')
...
function notifyApprovalNeeded(id) {
  ...
  // Sistem bildirimi
  admins.forEach(a => {
    window.addNotif?.('💰', '"' + o.name + '" ödemesi onayınızı bekliyor — ₺' + Math.round(_odmToTRY(o.amount||0, o.currency||'TRY', o)).toLocaleString('tr-TR'), 'warn', 'odemeler');
  });
...
Duay Global LLC`;
  ...
  mo.innerHTML = '<div class="moc" style="max-width:480px">'
    ...
    + '<textarea class="fi" id="notif-email" rows="8" style="resize:none;font-size:11px">' + window._esc(emailBody) + '</textarea></div>'
    + '<div class="mf">'
    + '<button class="btn" onclick="this.closest(\'.mo\').remove()">Kapat</button>'
...
window._odmExportHesapOzetiPDF = function() {
  try {
    ...
    var tahTRY = tah.reduce(function(s, t) { return s + _odmToTRY(parseFloat(t.amount) || 0, t.currency || 'TRY', t); }, 0);
    var odmTRY = odm.reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount) || 0, o.currency || 'TRY', o); }, 0);
...
window._odmYasDagilimRaporu = function() {
  ...
  odm.forEach(function(o) { var diff = Math.ceil((new Date(today) - new Date(o.due)) / 86400000); var g = diff <= 30 ? 0 : diff <= 60 ? 1 : diff <= 90 ? 2 : 3; groups[g].items.push({ name: o.name || o.cariName || '—', amt: _odmToTRY(parseFloat(o.amount) || 0, o.currency || 'TRY', o), diff: diff }); });
...
window._odmSLATakibi = function() {
  ...
  if (!odm.length) { html += '<div style="text-align:center;padding:32px;color:var(--t3)">Gecikmiş kayıt yok ✓</div>'; }
  else { odm.forEach(function(o) { var diff = Math.ceil((new Date(today) - new Date(o.due)) / 86400000); var c = diff > 60 ? '#dc2626' : diff > 30 ? '#d97706' : '#666'; var amt = _odmToTRY(parseFloat(o.amount) || 0, o.currency || 'TRY', o); html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 20px;border-bottom:0.5px solid var(--b)"><div><div style="font-size:12px;font-weight:500">' + window._esc(o.name || o.cariName || '—') + '</div><div style="font-size:10px;color:var(--t3)">Vade: ' + o.due + '</div></div><div style="text-align:right"><div style="color:' + c + ';font-weight:600;font-size:12px">' + diff + ' gün gecikmiş</div><div style="font-size:10px;color:var(--t3)">₺' + Math.round(amt).toLocaleString('tr-TR') + '</div></div></div>'; }); }
...
window._odmHatirlatmaTaslagi = function() {
  ...
  var taslak = window.DUAY_MAIL_GET('odeme_hatirlatma', 'tr', { cariName: o.cariName, due: o.due, sym: sym, amount: (parseFloat(o.amount) || 0).toLocaleString('tr-TR'), name: o.name, diff: diff });
...
window._renderOdmAnaliz = function() {
  ...
  var gecikTL = gecik.reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount || 0), o.currency || 'TRY', o); }, 0);
  ...
  var yaklasanTL = yaklasan.reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount || 0), o.currency || 'TRY', o); }, 0);
  var bekTah = tah.filter(function(t) { return !t.collected; });
  var bekTahTL = bekTah.reduce(function(s, t) { return s + _odmToTRY(parseFloat(t.amount || 0), t.currency || 'TRY', t); }, 0);
  var catMap = {}; odm.forEach(function(o) { if (o.paid) return; var c = o.cat || 'diger'; if (!catMap[c]) catMap[c] = 0; catMap[c] += _odmToTRY(parseFloat(o.amount || 0), o.currency || 'TRY', o); });
...
window._cariSupplierEvalModalAc = function(entry, onComplete) {
  var _s = function(v) { return String(v == null ? '' : v).replace(/[&<>"']/g, function(c){ return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]; }); };
  var ex = document.getElementById('cari-supplier-eval-modal'); if (ex) ex.remove();
  var html =
    '<div id="cari-supplier-eval-modal" style="position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:10000;display:flex;align-items:center;justify-content:center">' +
    '<div style="background:var(--sf,#fff);color:var(--t,#111);width:560px;max-width:92vw;max-height:90vh;overflow-y:auto;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.15);padding:24px;font-family:inherit">' +
      '<div style="font-size:15px;font-weight:600;margin-bottom:4px">Personel Değerlendirmesi</div>' +
      '<div style="font-size:11px;color:var(--t3,#666);margin-bottom:20px;letter-spacing:.03em">Firma: <b>' + _s(entry.name || '') + '</b> — onaya göndermeden önce zorunlu</div>' +
      '<label style="font-size:11px;letter-spacing:.05em;color:var(--t2,#333);text-transform:uppercase;display:block">Davranış & Güvenilirlik Notu <span style="color:#E0574F">*</span></label>' +
      '<textarea id="csev-davranis" rows="4" maxlength="2000" placeholder="Satıcının yaklaşımı, iletişim kalitesi, güven hissi, pazarlık tarzı... (min 100 karakter)" style="width:100%;box-sizing:border-box;padding:10px;border:0.5px solid var(--b,#CBD5E1);border-radius:8px;font-size:13px;margin:6px 0 4px;resize:vertical;font-family:inherit;background:var(--sf,#fff);color:var(--t,#111)"></textarea>' +
      '<div id="csev-davranis-count" style="font-size:10px;color:#999;text-align:right;margin-bottom:16px">0 / 100</div>' +
      ...
      '<div style="display:flex;gap:20px;margin-bottom:6px"><label style="font-size:13px;cursor:pointer"><input type="radio" name="csev-guv" value="yes"> Evet</label><label style="font-size:13px;cursor:pointer"><input type="radio" name="csev-guv" value="no"> Hayır</label></div>' +
      '<textarea id="csev-guv-acik" rows="2" maxlength="500" placeholder="Açıklama (min 50 karakter)" style="width:100%;box-sizing:border-box;padding:10px;border:0.5px solid var(--b,#CBD5E1);border-radius:8px;font-size:13px;margin-bottom:4px;resize:vertical;font-family:inherit;background:var(--sf,#fff);color:var(--t,#111)"></textarea>' +
      '<div id="csev-guv-count" style="font-size:10px;color:#999;text-align:right;margin-bottom:16px">0 / 50</div>' +
      ...
      '<div style="display:flex;gap:20px;margin-bottom:6px"><label style="font-size:13px;cursor:pointer"><input type="radio" name="csev-uzn" value="yes"> Evet</label><label style="font-size:13px;cursor:pointer"><input type="radio" name="csev-uzn" value="no"> Hayır</label></div>' +
      '<textarea id="csev-uzn-acik" rows="2" maxlength="500" placeholder="Açıklama (min 50 karakter)" style="width:100%;box-sizing:border-box;padding:10px;border:0.5px solid var(--b,#CBD5E1);border-radius:8px;font-size:13px;margin-bottom:4px;resize:vertical;font-family:inherit;background:var(--sf,#fff);color:var(--t,#111)"></textarea>' +
      '<div id="csev-uzn-count" style="font-size:10px;color:#999;text-align:right;margin-bottom:20px">0 / 50</div>' +
      '<div style="display:flex;gap:8px;justify-content:flex-end">' +
        '<button id="csev-cancel" style="padding:9px 18px;border:0.5px solid var(--b,#CBD5E1);background:transparent;color:var(--t2,#333);border-radius:8px;font-size:13px;cursor:pointer;font-family:inherit">İptal</button>' +
        '<button id="csev-submit" style="padding:9px 18px;border:none;background:#111;color:#fff;border-radius:8px;font-size:13px;cursor:pointer;font-weight:500;font-family:inherit">Onaya Gönder</button>' +
      '</div>' +
    '</div></div>';
  var wrap = document.createElement('div');
  wrap.innerHTML = html;
  ...
  /* Live karakter sayaç — yetersiz gri, yeterli yeşil */
  var _updCount = function(taId, countId, min) {
    ...
    var upd = function() {
      ...
      cnt.textContent = len + ' / ' + min;
      cnt.style.color = ta.value.trim().length >= min ? '#1A8D6F' : '#999';
    };
    ta.addEventListener('input', upd);
    upd();
  };
  _updCount('csev-davranis', 'csev-davranis-count', 100);
  _updCount('csev-guv-acik', 'csev-guv-count', 50);
  _updCount('csev-uzn-acik', 'csev-uzn-count', 50);

  document.getElementById('csev-cancel').onclick = function() { document.getElementById('cari-supplier-eval-modal').remove(); };
  document.getElementById('csev-submit').onclick = function() {
    var davranis = (document.getElementById('csev-davranis').value || '').trim();
    var guvR = (document.querySelector('input[name="csev-guv"]:checked') || {}).value;
    var guvAcik = (document.getElementById('csev-guv-acik').value || '').trim();
    var uznR = (document.querySelector('input[name="csev-uzn"]:checked') || {}).value;
    var uznAcik = (document.getElementById('csev-uzn-acik').value || '').trim();

    var eksikler = [];
    if (davranis.length < 100) eksikler.push('Davranış notu (min 100, şu an ' + davranis.length + ')');
    if (!guvR) eksikler.push('Güvenlik sorusu');
    if (guvAcik.length < 50) eksikler.push('Güvenlik açıklaması (min 50, şu an ' + guvAcik.length + ')');
    if (!uznR) eksikler.push('Uzun vade sorusu');
    if (uznAcik.length < 50) eksikler.push('Uzun vade açıklaması (min 50, şu an ' + uznAcik.length + ')');
    ...
    entry.supplierEvaluation = {
      ...
      guvenliMi: guvR === 'yes',
      guvenliAciklama: guvAcik,
      uzunVadeMi: uznR === 'yes',
      uzunVadeAciklama: uznAcik,
      userId: cu.id || cu.uid || 'bilinmiyor',
      userName: cu.name || cu.displayName || cu.email || 'bilinmiyor',
      timestamp: new Date().toISOString()
    };
    ...
    + '<div style="flex:1;overflow-y:auto;padding:14px 16px">'
      ...
      + '<div id="qc-panel-genel" class="qc-panel">'
        + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">'
          ...
          + '<div><div class="fl">VERGİ DAİRESİ *</div><input class="fi" id="qc-taxoffice" value="' + window._esc(c?.taxOffice || '') + '" placeholder="Vergi dairesi"></div>'
          + '<div><div class="fl">ÜLKE *</div><input class="fi" id="qc-country" value="' + window._esc(c?.country || '') + '" placeholder="Türkiye"></div>'
          + '<div><div class="fl">ŞEHİR</div><input class="fi" id="qc-city" value="' + window._esc(c?.city || '') + '"></div>'
          + '<div><div class="fl">TELEFON</div><input class="fi" id="qc-phone" value="' + window._esc(c?.phone || '') + '"></div>'
          + '<div><div class="fl">E-POSTA</div><input class="fi" type="email" id="qc-email" value="' + window._esc(c?.email || '') + '"></div>'
          + '<div><div class="fl">WEB</div><input class="fi" id="qc-web" value="' + window._esc(c?.web || '') + '" placeholder="www.firma.com"></div>'
          + '<div><div class="fl">POSTA KODU</div><input class="fi" id="qc-zip" value="' + window._esc(c?.zip || '') + '"></div>'
        + '</div>'
        + '<div style="margin-top:10px"><div class="fl">ADRES</div><textarea class="fi" id="qc-address" rows="2" style="resize:none">' + window._esc(c?.address || '') + '</textarea></div>'
      + '</div>'
      // Finansal sekmesi
      + '<div id="qc-panel-finans" class="qc-panel" style="display:none">'
        + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">'
          ...
          + '<div><div class="fl">BANKA ADI</div><input class="fi" id="qc-bank" value="' + window._esc(c?.bankName || '') + '"></div>'
          + '<div><div class="fl">IBAN</div><input class="fi" id="qc-iban" value="' + window._esc(c?.iban || '') + '"></div>'
          + '<div><div class="fl">SWIFT</div><input class="fi" id="qc-swift" value="' + window._esc(c?.swift || '') + '"></div>'
        + '</div>'
      + '</div>'
      // Kişiler sekmesi
        ...
        + '<div id="qc-contacts">' + (function() {
            ...
            return contacts.map(function(ct, i) {
              return '<div class="qc-contact-row" style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 30px;gap:6px;margin-bottom:6px">'
                + '<input class="fi qc-ct-name" placeholder="Ad Soyad" value="' + window._esc(ct.name || '') + '" style="font-size:11px;padding:5px">'
                + '<input class="fi qc-ct-pos" placeholder="Pozisyon" value="' + window._esc(ct.position || '') + '" style="font-size:11px;padding:5px">'
                + '<input class="fi qc-ct-phone" placeholder="Telefon" value="' + window._esc(ct.phone || '') + '" style="font-size:11px;padding:5px">'
                + '<input class="fi qc-ct-email" placeholder="E-posta" value="' + window._esc(ct.email || '') + '" style="font-size:11px;padding:5px">'
      ...
      + '<div id="qc-panel-belgeler" class="qc-panel" style="display:none">'
        ...
        + '<div id="qc-docs-list" style="margin-top:10px">' + (function() {
            ...
            return docs.map(function(d) { return '<div style="font-size:11px;padding:4px 0;border-bottom:1px solid var(--b)">📎 ' + window._esc(d.name || '?') + ' (' + (d.type || '—') + ')</div>'; }).join('') || '<div style="font-size:11px;color:var(--t3)">Belge yok</div>';
          })()
        + '</div>'
        + '<div style="margin-top:10px"><div class="fl">NOTLAR</div><textarea class="fi" id="qc-notes" rows="2" style="resize:none">' + window._esc(c?.notes || '') + '</textarea></div>'
      + '</div>'
...
function renderCari() {
  ...
  /* [CARI-KPI-REJECT-EKSIK-001] Reddedildi kategori eklendi, grid 10 kolona genişletildi — 8+6+7+2=23 tam eşleşme */
  statsEl.innerHTML = '<div style="display:grid;grid-template-columns:repeat(10,1fr);gap:6px;padding:8px 12px;border-bottom:1px solid var(--b);background:var(--sf)">'
    + '<div style="text-align:center;padding:6px;grid-column:span 2"><div style="font-size:16px;font-weight:700;color:var(--t)">' + all.length + '</div><div style="font-size:9px;color:var(--t3)">Toplam</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 2"><div style="font-size:16px;font-weight:700;color:#3B82F6">' + potCount + '</div><div style="font-size:9px;color:var(--t3)">🔵 Potansiyel</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 2"><div style="font-size:16px;font-weight:700;color:#F59E0B">' + aktCount + '</div><div style="font-size:9px;color:var(--t3)">🟡 Aktif</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 2"><div style="font-size:16px;font-weight:700;color:#16A34A">' + onayCount + '</div><div style="font-size:9px;color:var(--t3)">🟢 Onaylı</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 2"><div style="font-size:16px;font-weight:700;color:#DC2626">' + redCount + '</div><div style="font-size:9px;color:var(--t3)">🔴 Reddedildi</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 5"><div style="font-size:16px;font-weight:700;color:#DC2626">₺' + Math.round(toplamBorc).toLocaleString('tr-TR') + '</div><div style="font-size:9px;color:var(--t3)">Toplam Borç</div></div>'
    + '<div style="text-align:center;padding:6px;grid-column:span 5"><div style="font-size:16px;font-weight:700;color:#16A34A">₺' + Math.round(toplamAlacak).toLocaleString('tr-TR') + '</div><div style="font-size:9px;color:var(--t3)">Toplam Alacak</div></div>'
  + '</div>';
  ...
  function _getCariStatusColor(c) {
    ...
    var unpaid = cOdm.filter(function(o) { return !o.paid; }).reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
  ...
  if (pendingCari.length > 0) {
    ...
    html += pendingCari.map(function(c) {
      ...
      return '<div onclick="window._selectCari?.(' + c.id + ')" style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid #FDE68A;cursor:pointer;background:' + (isSel ? '#FEF3C7' : '#FFFBEB') + '">'
        ...
        + '<div style="flex:1;min-width:0">'
          + '<div style="font-size:12px;font-weight:600;color:#92400E;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + window._esc(c.name) + '</div>'
          + '<div style="font-size:10px;color:#B45309">' + (c.type === 'musteri' ? 'Müşteri' : c.type === 'tedarikci' ? 'Tedarikçi' : 'Diğer')
            + (createdByUser ? ' · ' + window._esc(createdByUser.name) : '') + ' · ' + (c.createdAt || '').slice(0, 10) + '</div>'
        + '</div>'
        + (isManager
  ...
  cont.innerHTML = '<div style="padding:20px">'
    ...
    + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">'
      ...
      + '<div style="display:flex;gap:6px;flex-wrap:wrap">'
        ...
        + '<button class="btn btns" onclick="event.stopPropagation();window.deleteCari(' + c.id + ')" style="font-size:11px;color:#DC2626;border-color:rgba(220,38,38,.3)" title="Cariyi sil">🗑 Sil</button>'
      + '</div>'
    + '</div>'
    // Onay durumu banner
    + (function() {
        if (c.status === 'pending_approval') {
          ...
          return '<div style="background:#FEF3C7;border:1px solid #FDE68A;border-radius:10px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between">'
            + '<div>'
              ...
              + '<div style="font-size:11px;color:#B45309;margin-top:2px">Oluşturan: ' + (createdByUser ? window._esc(createdByUser.name) : '—') + ' · ' + (c.createdAt || '').slice(0, 16) + '</div>'
            + '</div>'
            + (_isManagerO()
              ? '<div style="display:flex;gap:8px">'
        ...
        if (c.status === 'active' && c.approvedBy) {
          ...
          return '<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:10px;padding:8px 16px;margin-bottom:16px;display:flex;align-items:center;gap:8px">'
            ...
            + '<span style="font-size:10px;color:#15803D">' + (approverUser ? window._esc(approverUser.name) : '') + ' · ' + (c.approvedAt || '').slice(0, 16) + '</span>'
          + '</div>';
        }
        return '';
      })()
    ...
    + (function() {
        ...
        }).reduce(function(sum, o) { return sum + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
            ...
            return '<div style="display:grid;grid-template-columns:80px 1fr 80px 60px 70px 80px 70px;padding:6px 14px;background:var(--s2);border-bottom:1px solid var(--b);font-size:9px;font-weight:700;color:var(--t3);text-transform:uppercase"><div>Tarih</div><div>Açıklama</div><div style="text-align:right">Tutar</div><div>Döviz</div><div style="text-align:right">Kur</div><div style="text-align:right">TL Tutar</div><div style="text-align:right">Durum</div></div>'
              + (hareketler.length ? hareketler.slice(0, 30).map(function(h, i) {
                  ...
                  return '<div style="display:grid;grid-template-columns:80px 1fr 80px 60px 70px 80px 70px;padding:5px 14px;border-bottom:1px solid var(--b);font-size:11px;background:' + (i % 2 ? 'var(--s2)' : 'var(--sf)') + '">'
                    ...
                    + '<div style="color:var(--t)">' + window._esc(h.name || '—') + '</div>'
...
window._exportCariPDF = function() {
  ...
  w.document.write('<!DOCTYPE html><html><head><title>Cari Ekstre — ' + window._esc(c.name) + '</title>'
...
window._tahToggleQuickView = function(id) {
  ...
  qv.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;font-size:11px">'
    ...
    + '<div>'
      ...
      + (assigned ? '<div style="margin-bottom:4px"><span style="color:var(--t3)">Sorumlu:</span> ' + window._esc(assigned.name) + '</div>' : '')
      + '<div style="margin-bottom:4px"><span style="color:var(--t3)">Vade:</span> ' + (o.due || '—') + '</div>'
      + (o.cariName ? '<div style="margin-bottom:4px"><span style="color:var(--t3)">Cari:</span> ' + window._esc(o.cariName) + '</div>' : '')
      + (o.ref ? '<div style="margin-bottom:4px"><span style="color:var(--t3)">Referans No:</span> ' + window._esc(o.ref) + '</div>' : '')
      + (o.note ? '<div style="margin-bottom:4px"><span style="color:var(--t3)">Not:</span> ' + window._esc(o.note) + '</div>' : '')
    + '</div>'
    + '<div>'
...
function openOdmTrashPanel() {
  ...
  function _trashRow(item, type) {
    ...
    return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid var(--b)">'
      ...
      + '<div style="flex:1;min-width:0">'
        + '<div style="font-size:12px;font-weight:600;color:var(--t);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + window._esc(item.name || '—') + '</div>'
...
function _buildCariStatement(cariId, opts) {
  ...
  odm.forEach(function(o) {
    hareketler.push({
      ...
      currency: o.currency || 'TRY', amountTRY: _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o), kurRate: o.kurRate || 1,
  ...
  tah.forEach(function(t) {
    hareketler.push({
      ...
      currency: t.currency || 'TRY', amountTRY: _odmToTRY(parseFloat(t.amount||0), t.currency||'TRY', t), kurRate: t.kurRate || 1,
...
function openCariStatement(cariId, format) {
  ...
  st.hareketler.forEach(function(h, i) {
    ...
    rowsHTML += '<td style="padding:5px 8px;font-size:11px;color:var(--t)">' + window._esc(h.name) + '</td>';
    // Format user/admin ekstra sütunlar
    if (format !== 'musteri') {
      rowsHTML += '<td style="padding:5px 8px;font-size:10px;color:var(--t3)">' + window._esc(h.docNo) + '</td>';
    }
    ...
    if (allChanges.length) {
      ...
      allChanges.slice(0, 10).forEach(function(ch) {
        ...
        auditHTML += '<div style="font-size:10px;padding:3px 0;border-bottom:1px solid #FED7AA;color:#78350F">'
          + '<b>' + (ch.ts || '').slice(0, 16) + '</b> · ' + (byUser?.name || '—') + ' · <i>' + window._esc(ch.itemName || '') + '</i> → ' + ch.changes.join(', ')
          + '</div>';
      });
      auditHTML += '</div>';
    }
  ...
  mo.innerHTML = '<div class="moc" style="max-width:900px;padding:0;border-radius:16px;overflow:hidden">'
    ...
    + '<div>'
      ...
      + '<div style="font-size:11px;opacity:.7;margin-top:2px">' + formatLabel + ' — ' + window._esc(c.name) + '</div>'
    + '</div>'
    + '<div style="display:flex;gap:6px;align-items:center">'
    ...
    + (format === 'musteri' ? '<div style="padding:16px 24px;border-top:1px solid var(--b);display:grid;grid-template-columns:1fr 1fr;gap:40px;font-size:10px;color:var(--t3)">'
      ...
      + '<div><div style="margin-bottom:30px">Onaylayan:</div><div style="border-top:1px solid var(--t3);padding-top:4px">' + window._esc(c.name) + '</div></div>'
    + '</div>' : '')
...
/* DEMO-REMOVE-001: _insertCariDemoData fn + window export silindi (~57 satır demo cari/odm/tahsilat injection) */

/* DEMO-REMOVE-001: _temizleDemoVeri fn + JSDoc + window export silindi (~37 satır kurRate===38.50 demo soft-delete) */
...
function _calcAdvancedRiskScore(cariName) {
  ...
  var unpaid = odm.filter(function(o) { return !o.paid; }).reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
...
window.openProfitAnalysis = function() {
  ...
  var positions = cariList.map(function(c) {
    var cOdm = odm.filter(function(o) { return o.cariName === c.name; }).reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
    var cTah = tah.filter(function(t) { return t.cariName === c.name; }).reduce(function(s, t) { return s + _odmToTRY(parseFloat(t.amount||0), t.currency||'TRY', t); }, 0);
  ...
  mo.innerHTML = '<div class="moc" style="max-width:640px;padding:0;border-radius:14px;overflow:hidden">'
    ...
    + topProfit.map(function(p) {
        return '<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:12px;border-bottom:1px solid var(--b)"><span>' + window._esc(p.name) + '</span><span style="color:#16A34A;font-weight:600">+₺' + Math.round(p.net).toLocaleString('tr-TR') + '</span></div>';
      }).join('')
    ...
    + topLoss.map(function(p) {
        return '<div style="display:flex;justify-content:space-between;padding:4px 0;font-size:12px;border-bottom:1px solid var(--b)"><span>' + window._esc(p.name) + '</span><span style="color:#DC2626;font-weight:600">₺' + Math.round(p.net).toLocaleString('tr-TR') + '</span></div>';
      }).join('')
    + '</div></div>';
  document.body.appendChild(mo);
  mo.addEventListener('click', function(e) { if (e.target === mo) mo.remove(); });
...
function _checkDurationMismatch() {
  ...
  var odm30 = odm.filter(function(o) { return o.due && o.due >= todayStr && o.due <= d30; })
    .reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
  var tah30 = tah.filter(function(t) { return t.due && t.due >= todayStr && t.due <= d30; })
    .reduce(function(s, t) { return s + _odmToTRY(parseFloat(t.amount||0), t.currency||'TRY', t); }, 0);
  ...
  for (var i = 0; i < 30; i++) {
    ...
    var dayOdm = odm.filter(function(o) { return o.due === ds; }).reduce(function(s, o) { return s + _odmToTRY(parseFloat(o.amount||0), o.currency||'TRY', o); }, 0);
    var dayTah = tah.filter(function(t) { return t.due === ds; }).reduce(function(s, t) { return s + _odmToTRY(parseFloat(t.amount||0), t.currency||'TRY', t); }, 0);
...
window._assignCariReview = function(cariId) {
  ...
  mo.innerHTML = '<div class="moc" style="max-width:400px;padding:0;border-radius:14px;overflow:hidden">'
    + '<div style="padding:14px 20px;border-bottom:1px solid var(--b);font-size:14px;font-weight:700;color:var(--t)">👁 Cari İncelet — ' + window._esc(c.name) + '</div>'
    + '<div style="padding:16px 20px;display:flex;flex-direction:column;gap:12px">'
    + '<div><div class="fl">İNCELEYECEK KİŞİ *</div><select class="fi" id="cr-user">'
    + users.map(function(u) { return '<option value="' + u.id + '">' + window._esc(u.name) + ' (' + u.role + ')</option>'; }).join('')
    + '</select></div>'
