...
function openStokModal(dir,editId){
  ...
  const su=g('stk-user');if(su)su.innerHTML=users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');
  const zu=g('stk-zimmet-user');if(zu)zu.innerHTML=users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');
