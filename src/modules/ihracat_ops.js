...
window._ihrDetayTab = function(tab, id) {
  ...
  if (tab === 'evraklar') {
    ...
    evH += '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px">';
    Object.keys(EVRAK_TUR).forEach(function(k) {
      var v = EVRAK_TUR[k]; var ev = evraklar.find(function(e) { return e.tur === k; });
      var durum = ev ? (EVRAK_DURUM[ev.durum] || EVRAK_DURUM.taslak) : { l: 'Yok', c: '#9CA3AF', bg: 'rgba(156,163,175,.1)' };
      evH += '<div style="border:0.5px solid var(--b);border-radius:8px;padding:10px">';
      evH += '<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="font-size:11px;font-weight:500">' + _esc(v.l) + '</span><span style="font-size:9px;padding:1px 7px;border-radius:3px;background:' + durum.bg + ';color:' + durum.c + '">' + durum.l + '</span></div>';
      evH += '<div style="display:flex;gap:4px;flex-wrap:wrap">';
      evH += '<button class="btn btns" onclick="window._ihrPdfOnizle(\'' + d.id + '\',\'' + k + '\',null)" style="font-size:10px;padding:2px 8px">Görüntüle</button>';
      if (ev && ev.dosya_url) evH += '<a href="' + _esc(ev.dosya_url) + '" target="_blank" class="btn btns" style="font-size:10px;padding:2px 8px;text-decoration:none">İndir</a>';
      if (!ev) evH += '<button class="btn btns" onclick="window._ihrEvrakOlustur(\'' + d.id + '\',\'' + k + '\')" style="font-size:10px;padding:2px 8px">Oluştur</button>';
      if (ev) evH += '<button class="btn btns" onclick="window._ihrEvrakDuzenle(\'' + ev.id + '\')" style="font-size:10px;padding:2px 8px">Düzenle</button>';
      evH += '</div></div>';
    });
    evH += '</div></div>';
    c.innerHTML = evH; return;
  }
  if (tab === 'gumrukcu') {
...
function _ihrDetayRenderUrunler(d, el) {
  ...
  h += '<span style="font-size:10px;color:var(--t3);margin-left:auto">Son: ' + _now().slice(11, 16) + '</span>';
  h += '</div>';

  /* SATIR 2: Araçlar */
  h += '<div style="display:flex;align-items:center;gap:6px;padding:6px 0;border-bottom:0.5px solid var(--b);flex-wrap:wrap">';
  h += '<input class="fi" id="ihr-urun-ara" placeholder="Ara..." oninput="event.stopPropagation();window._ihrUrunAramaQ=this.value;window.renderIhracatOps()" value="' + _esc(_aramaQ) + '" style="width:180px;font-size:11px" onclick="event.stopPropagation()">';
  h += '<select class="fi" onchange="event.stopPropagation();window._ihrFiltrele(\'tedarikciAd\',this.value)" style="font-size:10px;padding:3px 6px" onclick="event.stopPropagation()"><option value="">Tedarikçi</option>';
  uniq('tedarikciAd').sort().forEach(function(v) { h += '<option value="' + _esc(v) + '"' + (_filtreler.tedarikciAd === v ? ' selected' : '') + '>' + _esc(v) + '</option>'; });
  h += '</select>';
  h += '<select class="fi" onchange="event.stopPropagation();window._ihrFiltrele(\'etiket_rengi\',this.value)" style="font-size:10px;padding:3px 6px" onclick="event.stopPropagation()"><option value="">Etiket</option>';
  ['Mavi', 'Pembe', 'Sarı', 'Yeşil', 'Mor', 'Turuncu'].forEach(function(v) { h += '<option value="' + v + '"' + (_filtreler.etiket_rengi === v ? ' selected' : '') + '>' + v + '</option>'; });
  h += '</select>';
  h += '<select class="fi" onchange="event.stopPropagation();window._ihrFiltrele(\'once_yukle\',this.value)" style="font-size:10px;padding:3px 6px" onclick="event.stopPropagation()"><option value="">Yükle</option>';
  uniq('once_yukle').forEach(function(v) { h += '<option value="' + _esc(v) + '"' + (_filtreler.once_yukle === v ? ' selected' : '') + '>' + _esc(v) + '</option>'; });
  h += '</select>';
  h += '<select class="fi" onchange="event.stopPropagation();window._ihrFiltrele(\'fatura_turu\',this.value)" style="font-size:10px;padding:3px 6px" onclick="event.stopPropagation()"><option value="">Fatura Türü</option>';
  ...
  /* Kolon tanımları */
  var KOLONLAR = [
    { k: 'tedarikciAd',       l: 'Tedarikçi',        w: 110, filtre: true },
    { k: 'proforma_id',       l: 'Proforma ID',       w: 90,  filtre: false },
    { k: 'pi_link',           l: 'PI Link',           w: 60,  filtre: false },
    { k: 'satis_siparis_id',  l: 'Satış Sipariş',     w: 90,  filtre: false },
    { k: 'alis_fatura_no',    l: 'Alış Fatura No',    w: 90,  filtre: false },
    ...
    { k: 'kdv_orani',         l: 'KDV%',              w: 45,  filtre: true },
    { k: 'kdv_tutar',         l: 'KDV Tutarı',        w: 80,  filtre: false },
    { k: 'kdv_dahil',         l: 'KDV Dahil',         w: 80,  filtre: false },
    { k: 'teslim_tarihi',     l: 'Teslim Tarihi',     w: 80,  filtre: true },
  ...
  KOLONLAR.forEach(function(kol) {
    h += '<th style="width:' + kol.w + 'px;min-width:' + kol.w + 'px">';
    h += '<div style="display:flex;align-items:center;gap:2px"><span>' + kol.l + '</span>';
    if (kol.filtre) {
      var aktif = _filtreler[kol.k] ? '\ud83d\udfe2' : '\u25be';
      h += '<select onchange="window._ihrFiltrele(\'' + kol.k + '\',this.value)" style="border:none;background:transparent;font-size:9px;cursor:pointer;color:' + (_filtreler[kol.k] ? '#185FA5' : 'var(--t3)') + '">';
      h += '<option value="">' + aktif + '</option><option value="">(Tümü)</option>';
      uniq(kol.k).sort().forEach(function(v) { h += '<option value="' + _esc(v) + '"' + (_filtreler[kol.k] === v ? ' selected' : '') + '>' + _esc(v) + '</option>'; });
      h += '</select>';
    }
    h += '</div></th>';
  });
  h += '<th style="width:50px"></th></tr></thead>';
  ...
  urunler.sort(function(a, b) { return (a.konteyner_sira || 99) - (b.konteyner_sira || 99); }).forEach(function(u) {
    ...
    var eksikHsSatir = !u.hs_kodu;
    var rowBg = tutarsiz ? '#FAEEDA22' : eksikHsSatir ? '#FCEBEB11' : 'inherit';
    ...
    h += '<td style="text-align:right;font-size:10px;font-family:monospace;font-weight:500">' + kdvDahil.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + '</td>';
    ...
    h += '<td onclick="window._ihrInlineEdit(this,\'' + u.id + '\',\'siparis_id\')" style="cursor:text;font-size:10px;font-family:monospace">' + _esc(u.siparis_id || '—') + '</td>';
...
window._ihrSatinalmaCek = function(dosyaId) {
  ...
  uygun.forEach(function(s) {
    satirHtml += '<tr><td><input type="checkbox" class="sc-chk" data-id="' + s.id + '"></td>';
    satirHtml += '<td style="font-size:10px">' + _esc(s.tedarikci || '—') + '</td>';
    satirHtml += '<td style="font-weight:500">' + _esc(s.urun || s.aciklama || '—') + '</td>';
    satirHtml += '<td style="text-align:right">' + (s.miktar || 0) + ' ' + _esc(s.birim || '') + '</td>';
    satirHtml += '<td style="text-align:right;font-family:monospace">' + (s.birimFiyat || 0) + ' ' + _esc(s.doviz || 'USD') + '</td>';
    satirHtml += '<td style="font-size:10px;font-family:monospace">' + _esc((s.tarih || s.createdAt || '—').slice(0, 10)) + '</td></tr>';
  });
  mo.innerHTML = '<div class="moc" style="max-width:700px;padding:0;border-radius:14px;overflow:hidden">'
    + '<div style="padding:14px 20px;border-bottom:1px solid var(--b);display:flex;align-items:center;justify-content:space-between"><div style="font-size:14px;font-weight:600">Satınalma\'dan Ürün Çek</div><button onclick="document.getElementById(\'mo-satinalma-cek\')?.remove()" style="background:none;border:none;cursor:pointer;font-size:18px;color:var(--t3)">x</button></div>'
    + '<div style="padding:18px 20px"><input type="hidden" id="sc-dosya-id" value="' + dosyaId + '"><div style="font-size:11px;color:var(--t2);margin-bottom:10px">Seçilen satınalma kayıtları ihracat dosyasına eklenecek.</div>'
    + '<div style="max-height:350px;overflow-y:auto"><table class="tbl" style="font-size:11px"><thead><tr><th style="width:28px"><input type="checkbox" onchange="document.querySelectorAll(\'.sc-chk\').forEach(function(c){c.checked=this.checked}.bind(this))"></th><th>Tedarikçi</th><th>Ürün</th><th>Miktar</th><th>Birim Fiyat</th><th>Tarih</th></tr></thead><tbody>' + satirHtml + '</tbody></table></div></div>'
...
window._ihrTopluDuzenle = function(dosyaId) {
  ...
  mo.innerHTML = '<div class="moc" style="max-width:480px;padding:0;border-radius:14px;overflow:hidden">'
    + '<div style="padding:14px 20px;border-bottom:1px solid var(--b);font-size:14px;font-weight:600">' + seciliIdler.length + ' Ürün — Toplu Düzenle</div>'
    + '<div style="padding:18px 20px"><div style="font-size:11px;color:var(--t2);margin-bottom:12px">Doldurulan alanlar seçili tüm ürünlere uygulanacak. Boş bırakılan alanlar değiştirilmez.</div>'
    + '<div style="display:flex;flex-direction:column;gap:10px">'
    ...
    + '<div class="fg"><div class="fl">Teslim Yeri</div><input class="fi" id="te-teslim-yer"></div>'
    + '<div class="fg"><div class="fl">Fatura Türü</div><select class="fi" id="te-fatura"><option value="">— Değiştirme —</option><option>İhraç Kayıtlı KDV\'li</option><option>İhraç Kayıtlı KDV\'siz</option><option>KDV Muaf</option></select></div>'
    + '<div class="fg"><div class="fl">Menşe Ülke</div><input class="fi" id="te-mense"></div>'
    + '</div></div>'
    + '<div style="padding:12px 20px;border-top:1px solid var(--b);display:flex;gap:8px;justify-content:flex-end"><button class="btn btns" onclick="document.getElementById(\'mo-toplu-edit\')?.remove()">İptal</button><button class="btn btnp" onclick="window._ihrTopluDuzenleKaydet(' + JSON.stringify(seciliIdler) + ')">Uygula</button></div></div>';
  document.body.appendChild(mo); setTimeout(function() { mo.classList.add('open'); }, 10);
};
window._ihrTopluDuzenleKaydet = function(idler) {
  var urunler = _loadU();
  var degerler = { etiket_rengi: (_g('te-etiket') || {}).value, once_yukle: (_g('te-yukle') || {}).value, teslim_tarihi: (_g('te-teslim-tarih') || {}).value, teslim_yeri: ((_g('te-teslim-yer') || {}).value || '').trim(), fatura_turu: (_g('te-fatura') || {}).value, mense_ulke: ((_g('te-mense') || {}).value || '').trim() };
  idler.forEach(function(id) { var u = urunler.find(function(x) { return String(x.id) === String(id); }); if (!u) return; for (var k in degerler) { if (degerler[k]) u[k] = degerler[k]; } u.updatedAt = _now(); });
