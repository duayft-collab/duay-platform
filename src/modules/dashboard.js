...
const S_LBL = 'font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:' + T3;
const S_VAL = 'font-size:18px;font-weight:500';
const S_SUB = 'font-size:9px;color:' + T3;
const S_SEC = 'font-size:9px;font-weight:500;text-transform:uppercase;letter-spacing:0.05em;color:' + T3 + ';margin-bottom:5px;display:flex;align-items:center;gap:6px';
...
function _renderBanner() {
  ...
  const chip = (label, val) => '<span style="padding:5px 10px;background:rgba(255,255,255,0.08);border-radius:6px;font-size:9px;color:#E6F1FB">' + label + ' <b>' + val + '</b></span>';

  return '<div style="background:#042C53;border-radius:9px;padding:11px 16px;display:flex;align-items:center;justify-content:space-between">'
    + '<div>'
    + '<div style="font-size:13px;color:#E6F1FB;font-weight:500">Günaydın, ' + name + '</div>'
    + '<div style="font-size:10px;color:#85B7EB">' + dayStr + ' · ' + pendingCount + ' onay bekliyor · Sistem skoru ' + _esc(skorVal) + '</div>'
    + '</div>'
...
function _renderFinans() {
  ...
  h += '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px">';
...
function _renderKPI() {
  ...
  h += '<div style="' + S_WK + ';display:flex;align-items:center;gap:8px">'
    + _ring(konvPct, 36, 3, konvColor)
    + '<div><div style="font-size:10px;font-weight:500;color:' + T1 + '">Satış Konversiyon</div>'
    + '<div style="font-size:8px;color:' + T3 + '">Hedef: ' + _pct(hedefKonv) + ' · Teklif→Sipariş</div>'
    + '<div style="font-size:8px;color:' + T3 + '">Fark: ' + (konvPct - hedefKonv >= 0 ? '+' : '') + _pct(konvPct - hedefKonv) + '</div>'
    + _bg('Hedef') + '</div></div>';
  ...
  barSvg += '</svg>';
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:4px">Haftalık Büyüme</div>'
    + barSvg
    + '<div style="font-size:8px;color:' + T3 + ';margin-top:3px;display:flex;justify-content:space-between">'
  ...
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:6px">Ekip Performans</div>';
  users.slice(0, 4).forEach(u => {
    ...
    h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:8px;color:' + T2 + ';margin-bottom:1px">'
  ...
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:6px">Hedef vs Gerçek</div>';
  const hvg = [
...
function _renderEmythVanish() {
  let h = '<div style="display:grid;grid-template-columns:3fr 2fr;gap:7px">';

  // ── E-MYTH ──
  h += '<div>';
  h += '<div style="' + S_SEC + '">' + _esc('E-MYTH — SİSTEM KURUMU') + ' ' + _bp('SOP & Sistem') + '</div>';
  h += '<div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px">';
  ...
  h += '<div style="' + S_WK + ';display:flex;align-items:center;gap:8px">'
    + _ring(sopPct, 34, 2.5, sopColor)
    + '<div><div style="font-size:12px;font-weight:600;color:' + T1 + '">' + sopTam + '/' + Math.round(sopTop) + '</div>'
    + '<div style="font-size:8px;color:#E24B4A">' + (Math.round(sopTop) - sopTam) + ' eksik</div></div></div>';

  // Sistemsiz İş Oranı
  const sistemsiz = _lsn('ak_sistemsiz_oran');
  const sisColor = sistemsiz < 10 ? GREEN : AMBER;
  h += '<div style="' + S_WK + ';display:flex;align-items:center;gap:8px">'
    + _ring(sistemsiz, 34, 2.5, sisColor)
    + '<div><div style="font-size:12px;font-weight:600;color:' + T1 + '">' + _pct(sistemsiz) + '</div>'
    + '<div style="font-size:8px;color:#E24B4A">Hedef: %10 altı</div></div></div>';

  // Modül Skorları
  const skorlar = _lsj('ak_modul_skorlar', { nakit: 0, gorev: 0, katalog: 0, musteri: 0 });
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:4px">Modül Skorları</div>';
  Object.entries(skorlar).forEach(([k, v]) => {
    const b = v >= 80 ? _bg(_pct(v)) : v >= 60 ? _ba(_pct(v)) : _br(_pct(v));
    h += '<div style="display:flex;justify-content:space-between;align-items:center;font-size:9px;color:' + T2 + ';margin-bottom:2px">'
  ...
  h += '<div style="background:' + BG2 + ';border-radius:6px;padding:8px;text-align:center">'
    + '<div style="font-size:20px;font-weight:600;color:' + GREEN + '">' + _pct(otoOran) + '</div>'
    + '<div style="font-size:8px;color:' + T3 + '">Otomasyon</div></div>';
  h += '<div style="background:' + BG2 + ';border-radius:6px;padding:8px;text-align:center">'
    + '<div style="font-size:20px;font-weight:600;color:' + BLUE + '">' + Math.round(sahipsiz) + '</div>'
    + '<div style="font-size:8px;color:' + T3 + '">Sahipsiz Karar/hafta</div></div>';
  h += '</div>';
  ...
  h += '<div style="font-size:10px;color:' + T3 + ';margin-top:7px">Sahip müdahalesi düşükse sistem çalışıyor.</div>';
  h += '</div></div>';
...
function _renderNakit() {
  ...
  const tabBtn = (label, val) => {
    ...
    return '<span onclick="window._dashNakitTab(' + val + ')" style="cursor:pointer;padding:2px 8px;border-radius:4px;font-size:8px;'
  ...
  h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'
    + '<span style="font-size:10px;font-weight:500;color:' + T1 + '">Nakit Akışı Trendi</span>'
...
function _renderAlt() {
  ...
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:6px">Son Aktivite</div>';
  const ikonMap = {
  ...
  notifs.forEach(n => {
    ...
    h += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">'
      + '<div style="width:18px;height:18px;border-radius:50%;background:' + ic[0] + ';display:flex;align-items:center;justify-content:center;flex-shrink:0">'
      + '<span style="font-size:8px;color:' + ic[1] + '">●</span></div>'
      ...
      + '<span style="font-size:8px;color:' + T3 + ';white-space:nowrap">' + saat + '</span></div>';
  });
  h += '</div>';
  ...
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:6px">Aktif İddialar</div>';
  ...
  iddialar.forEach(id => {
    ...
    h += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">'
      + '<div style="width:18px;height:18px;border-radius:50%;background:' + avBg + ';display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:600;flex-shrink:0">' + harf + '</div>'
      + '<div style="flex:1;min-width:0"><div style="font-size:9px;color:' + T1 + ';overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + baslık + '</div>'
      + '<div style="display:flex;gap:4px;align-items:center;font-size:8px;color:' + T3 + '">' + kalanGun + 'g kaldı · ' + turBadge + '</div></div></div>';
  });
  h += '</div>';
  ...
  h += '<div style="' + S_WK + '">'
    + '<div style="font-size:10px;font-weight:500;color:' + T1 + ';margin-bottom:6px">Q2 Hedefleri</div>';
  ...
  h += '<div style="font-size:12px;color:#A32D2D;margin-bottom:6px">ℹ</div>';
  if (soz) {
    h += '<div style="font-size:10px;color:#E6F1FB;font-style:italic;line-height:1.5">' + _esc(soz.soz || soz.text || '') + '</div>'
      + '<div style="width:20px;height:1.5px;background:#378ADD;margin:6px 0"></div>'
      + '<div style="font-size:9px;color:#85B7EB">' + _esc(soz.yazar || soz.author || '') + '</div>';
  } else {
    h += '<div style="font-size:10px;color:#85B7EB;font-style:italic">Henüz söz eklenmemiş</div>';
  }
  h += '</div>';
