...
(function() {
  ...
  window._shipmentDocUiUploadFile = function(edId, slot) {
    ...
    inp.onchange = async function() {
      ...
      /* SHIPMENT-DOC-TIR-SHARE-001: uploadGroupId enjekte + SHARED slot check (V129) */
      if (newSlotMetas.length > 0) {
        /* uploadGroupId enjekte (her batch unique) */
        const uploadGroupId = (typeof _genUploadGroupId === 'function') ? _genUploadGroupId() : null;
        if (uploadGroupId) {
        ...
        const isSharedSlot = SHARED_SLOTS.indexOf(slot) >= 0;
        ...
        if (isSharedSlot && typeof window.loadExpectedDeliveries === 'function') {
          try {
            ...
            groupedEds = _findGroupedEds(edId, konteynerNo);
          } catch (e) { /* defensive: SHARED check başarısızsa V128 davranış */ }
        }
        /* Branch: multi-ED vs single-ED */
        if (groupedEds.length > 0) {
          /* Apply-modal göster, callback'te multi-ED save */
          _showApplyModal(edId, (ed && ed.productName) || '—', groupedEds, slot, successList, function(selectedEdIds) {
            const r = _saveToMultipleEds(selectedEdIds, slot, newSlotMetas, meta);
          ...
          const r = _saveToSingleEd(edId, slot, newSlotMetas, meta);
