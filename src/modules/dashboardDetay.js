...
const S_LBL = 'font-size:11px;font-weight:500;text-transform:uppercase;letter-spacing:0.04em;color:' + T3;
const S_VAL = 'font-size:21px;font-weight:500';
const S_SUB = 'font-size:11px;color:' + T3;

/* ── Badge ──────────────────────────────────────────────────── */
function _badge(text, bg, fg) { return '<span style="font-size:9px;padding:2px 7px;border-radius:4px;background:' + bg + ';color:' + fg + '">' + _esc(text) + '</span>'; }
...
function _chip(label, val) { return '<span style="padding:7px 13px;background:rgba(255,255,255,0.08);border-radius:6px;font-size:11px;color:#E6F1FB">' + label + ' <b>' + val + '</b></span>'; }
...
function _renderBanner(grup) {
  ...
  return '<div style="background:#042C53;border-radius:9px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between">'
    + '<div><div style="font-size:16px;color:#E6F1FB;font-weight:500">' + _esc(c.title) + '</div>'
    + '<div style="font-size:12px;color:#85B7EB">' + _esc(c.sub) + '</div></div>'
...
function _renderGrafik(grup) {
  ...
  const tabBtn = (label, val) => {
    ...
    return '<span onclick="window._detayGrafTab(' + val + ')" style="cursor:pointer;padding:3px 9px;border-radius:4px;font-size:9px;' + (active ? 'background:#E6F1FB;color:'+NAVY+';font-weight:600' : 'color:'+T3) + '">' + label + '</span>';
  };
  ...
  if (grup === 'finans') {
    ...
    h += '<div style="display:flex;gap:12px;font-size:10px;color:'+T3+';margin-top:4px">'
    ...
    h += '<div style="font-size:10px;color:'+T3+';margin-top:4px">Son 6 hafta · Tamamlanan görevler</div>';
  } else if (grup === 'katalog') {
    ...
    h += '<div style="font-size:10px;color:'+T3+';margin-top:4px">Son 6 hafta · Yeni ürünler</div>';
  } else {
    h += '<span style="font-size:10px;font-weight:500;color:' + T1 + '">Performans Trendi</span></div>';
    const tk = _loadTasks().filter(t => t.done && t.completedAt);
    const wk = _weeklyCount(tk, 'completedAt', 6);
    h += _svgLine(wk.map(w=>w.count), [], 400, 55);
    h += '<div style="font-size:10px;color:'+T3+';margin-top:4px">Son 6 hafta · Tamamlanan</div>';
  }
  return h + '</div>';
}
...
function _renderIkili(grup) {
  ...
  if (grup === 'finans') {
    ...
    h += _ring(konvPct, 48, 3, konvColor);
    h += '<div><div style="font-size:13px;font-weight:500;color:'+T1+'">Satış Konversiyon</div>'
      + '<div style="font-size:10px;color:'+T3+'">Hedef: '+_pct(hedefK)+' · Fark: '+(konvPct-hedefK>=0?'+':'')+_pct(konvPct-hedefK)+'</div></div></div>';
    // Hedef vs Gerçek
    const tah = _loadTah();
    const ayT = tah.filter(t=>t.createdAt&&_thisMonth(new Date(t.createdAt))).reduce((s,t)=>s+(parseFloat(t.amountTRY||t.amount)||0),0);
    ...
    items.forEach(it => { const c = _pctColor(it.p); h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+it.l+'</span><span style="color:'+c+'">'+_pct(it.p)+'</span></div>'+_bar(it.p,c)+'</div>'; });
    h += '</div>';
    ...
    h += '<div style="' + S_WK + '">';
    h += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">';
    h += _ring(sopPct, 48, 3, sopC);
    h += '<div><div style="font-size:13px;font-weight:500;color:'+T1+'">SOP Tamamlanma</div>'
      + '<div style="font-size:10px;color:'+T3+'">'+sopT+'/'+Math.round(sopTp)+'</div></div></div>';
    const del = _lsn('ak_delegasyon_skor'), mud = _lsn('ak_sahip_mudahale');
    h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>Delegasyon</span><span>'+_pct(del)+'</span></div>'+_bar(del,GREEN)+'</div>';
    h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>Sahip Müdahale</span><span>'+_pct(mud)+'</span></div>'+_bar(mud,'#E24B4A')+'</div>';
    h += '</div>';
    ...
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Görev Dağılımı</div>';
    [{ l:'Tamamlandı', p:Math.round(done/total*100), c:GREEN },{ l:'Devam', p:Math.round(devam/total*100), c:BLUE },{ l:'Gecikmiş', p:Math.round(gecik/total*100), c:RED }]
      .forEach(it => { h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+it.l+'</span><span>'+_pct(it.p)+'</span></div>'+_bar(it.p,it.c)+'</div>'; });
    h += '</div>';
    const kr = _loadKargo();
    const aktifK = kr.filter(k=>k.status!=='teslim'&&!k.isDeleted).length, teslim = kr.filter(k=>k.status==='teslim').length;
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Kargo Durumu</div>';
    [{ l:'Aktif', v:aktifK, c:BLUE },{ l:'Teslim', v:teslim, c:GREEN },{ l:'Toplam', v:kr.length, c:T1 }]
      .forEach(it => { h += '<div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:3px"><span>'+it.l+'</span><span style="font-weight:500;color:'+it.c+'">'+it.v+'</span></div>'; });
    h += '</div>';

  } else if (grup === 'katalog') {
    const ur = _loadUrunler(), ca = _loadCari();
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Ürün Kategorileri</div>';
    const cats = {}; ur.forEach(u => { const k = u.kategori || u.category || 'Diğer'; cats[k] = (cats[k]||0)+1; });
    Object.entries(cats).slice(0,5).forEach(([k,v]) => { const p = Math.round(v/ur.length*100); h += '<div style="margin-bottom:3px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+_esc(k)+'</span><span>'+v+'</span></div>'+_bar(p,BLUE)+'</div>'; });
    if (!Object.keys(cats).length) h += '<div style="font-size:10px;color:'+T3+';padding:6px 0">Kategori verisi yok</div>';
    h += '</div>';
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Cari Türleri</div>';
    const turler = {}; ca.forEach(c => { const k = c.tur || c.type || 'Diğer'; turler[k] = (turler[k]||0)+1; });
    Object.entries(turler).slice(0,5).forEach(([k,v]) => { const p = Math.round(v/ca.length*100); h += '<div style="margin-bottom:3px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+_esc(k)+'</span><span>'+v+'</span></div>'+_bar(p,NAVY)+'</div>'; });
    if (!Object.keys(turler).length) h += '<div style="font-size:10px;color:'+T3+';padding:6px 0">Tür verisi yok</div>';
    h += '</div>';

  } else {
    // Ekip
    const us = _loadUsers().filter(u=>u.status==='active'), tk = _loadTasks();
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Kullanıcı Performans</div>';
    us.slice(0,5).forEach(u => {
      const uT = tk.filter(t=>t.uid===u.id), done = uT.filter(t=>t.done).length, total = uT.length||1;
      const pct = Math.round(done/total*100);
      h += '<div style="margin-bottom:4px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+_esc((u.name||'').split(' ')[0])+'</span><span>'+_pct(pct)+'</span></div>'+_bar(pct,BLUE)+'</div>';
    });
    h += '</div>';
    const id = _loadIddia();
    ...
    h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">İddia Kazanma</div>';
    h += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">' + _ring(oran, 48, 3, oran>=60?GREEN:oran>=40?AMBER:'#E24B4A');
    h += '<div><div style="font-size:10px;color:'+T2+'">Kazanılan: '+kazanan+'</div><div style="font-size:10px;color:'+T2+'">Aktif: '+aktif+'</div></div></div>';
    const seri = id.filter(x=>x.durum==='kazanildi').sort((a,b)=>(b.bitisTarih||'').localeCompare(a.bitisTarih||'')).length;
    h += '<div style="font-size:10px;color:'+T3+'">Toplam kazanılan seri: '+seri+'</div>';
    h += '</div>';
  }
...
function _renderEmythQ2(grup) {
  ...
  if (grup === 'finans') {
    ...
    Object.entries(skorlar).forEach(([k,v]) => { const b = v>=80?_bg(_pct(v)):v>=60?_ba(_pct(v)):_br(_pct(v)); h += '<div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:2px"><span>'+_esc(k)+'</span>'+b+'</div>'; });
    h += '</div>';
    // Sağ: Vanish
    ...
    h += '<div style="background:'+BG2+';border-radius:6px;padding:6px;text-align:center"><div style="font-size:22px;font-weight:600;color:'+GREEN+'">'+_pct(oto)+'</div><div style="font-size:7px;color:'+T3+'">Otomasyon</div></div>';
    h += '<div style="background:'+BG2+';border-radius:6px;padding:6px;text-align:center"><div style="font-size:22px;font-weight:600;color:'+BLUE+'">'+Math.round(sah)+'</div><div style="font-size:7px;color:'+T3+'">Sahipsiz/hf</div></div></div>';
    h += '<div style="margin-bottom:3px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>Delegasyon</span><span>'+_pct(del)+'</span></div>'+_bar(del,GREEN)+'</div>';
    h += '<div><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>Müdahale</span><span>'+_pct(mud)+'</span></div>'+_bar(mud,'#E24B4A')+'</div>';
    h += '</div></div>';
  }
  ...
  h += '<div style="' + S_WK + '"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Q2 Hedefleri</div>';
  h += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 14px">';
  q2Items.forEach(it => { const c = it.p>=80?BLUE:it.p>=60?AMBER:'#E24B4A'; h += '<div style="margin-bottom:3px"><div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:1px"><span>'+it.l+'</span><span style="color:'+c+'">'+_pct(it.p)+'</span></div>'+_bar(it.p,c)+'</div>'; });
  h += '</div></div>';
  return h;
}
...
function _renderSidebar() {
  ...
  h += '<div style="' + S_WK + ';margin-bottom:7px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'
    + '<span style="font-size:13px;font-weight:500;color:'+T1+'">Onay Bekleyen</span>'+_br(String(pendO.length+pendT.length-pend.length+pend.length))+'</div>';
  pend.forEach(p => {
    const ad = _esc(p.cari || p.cariAd || p.supplier || '—');
    const tutar = '₺' + _fmt(parseFloat(p.amountTRY||p.amount)||0);
    h += '<div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:3px"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:120px">'+ad+'</span>'+_ba(tutar)+'</div>';
  });
  if (!pend.length) h += '<div style="font-size:10px;color:'+T3+'">Onay yok</div>';
  h += '<div style="font-size:9px;color:'+BLUE+';cursor:pointer;margin-top:4px" onclick="window.App?.nav?.(\'odemeler\')">Tümünü gör →</div></div>';

  // 2. Döviz Pozisyon
  const fxSym = {USD:'$',EUR:'€',TRY:'₺'};
  h += '<div style="' + S_WK + ';margin-bottom:7px"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Döviz Pozisyon</div>';
  ['USD','EUR','TRY'].forEach(cur => {
    ...
    const net = tahC - odmC;
    h += '<div style="display:flex;justify-content:space-between;font-size:10px;color:'+T2+';margin-bottom:3px"><span>'+cur+'</span><span style="font-weight:500;color:'+(net>=0?GREEN:RED)+'">'+(net>=0?'+':'-')+fxSym[cur]+_fmt(net)+'</span></div>';
  });
  h += '</div>';
  ...
  h += '<div style="' + S_WK + ';margin-bottom:7px"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Ekip Performans</div>';
  ...
  h += '<div style="' + S_WK + ';margin-bottom:7px"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Aktif İddialar</div>';
  iddialar.forEach(id => {
    ...
    h += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">'
      + '<div style="width:20px;height:20px;border-radius:50%;background:'+avColors[hash%avColors.length]+';display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:600;flex-shrink:0">'+harf+'</div>'
      + '<div style="flex:1;min-width:0"><div style="font-size:10px;color:'+T1+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+_esc(id.baslik||id.title||'')+'</div>'
      + '<div style="font-size:10px;color:'+T3+'">'+kalanGun+'g kaldı</div></div></div>';
  });
  if (!iddialar.length) h += '<div style="font-size:10px;color:'+T3+'">Aktif iddia yok</div>';
  h += '</div>';
  ...
  h += '<div style="' + S_WK + ';margin-bottom:7px"><div style="font-size:13px;font-weight:500;color:'+T1+';margin-bottom:6px">Son Aktivite</div>';
  notifs.forEach(n => {
    ...
    h += '<div style="display:flex;align-items:center;gap:5px;margin-bottom:3px">'
      + '<div style="width:15px;height:15px;border-radius:50%;background:'+ic[0]+';display:flex;align-items:center;justify-content:center;flex-shrink:0"><span style="font-size:7px;color:'+ic[1]+'">●</span></div>'
  ...
  if (!notifs.length) h += '<div style="font-size:10px;color:'+T3+'">Bildirim yok</div>';
  h += '</div>';
  ...
  if (soz) {
    h += '<div style="font-size:11px;color:#E6F1FB;font-style:italic;line-height:1.5">'+_esc(soz.soz||soz.text||'')+'</div>'
      ...
      + '<div style="font-size:9px;color:#85B7EB">'+_esc(soz.yazar||soz.author||'')+'</div>';
  } else { h += '<div style="font-size:10px;color:#85B7EB;font-style:italic">Henüz söz eklenmemiş</div>'; }
  h += '</div>';
