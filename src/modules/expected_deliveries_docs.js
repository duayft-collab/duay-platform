...
(function() {
  ...
  /* SHIPMENT-DOC-PROBE-001: Manager-only diagnostic — DevTools console output, modal YOK */
  ...
  window._shipmentDocProbe = function(edId) {
    ...
    const idx = _edFindIdx(list, edId);
    if (idx === -1) {
      console.warn('[PROBE] ED bulunamadı:', edId);
      if (typeof window.toast === 'function') window.toast('ED bulunamadı', 'err');
      return;
    }

    let sd = list[idx].shipmentDoc;

    /* Doc yoksa otomatik oluştur (idempotent test) */
    if (!sd) {
    ...
    console.log('Owner:    ' + window._shipmentDocMaskName(sd.history.length ? (sd.history[0].uname || '?') : 'Yusuf'));
