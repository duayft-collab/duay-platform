...
<div class="mo" id="mo-task" role="dialog" aria-modal="true" aria-labelledby="mo-tk-t">
  <div class="moc" style="width:580px;max-width:96vw;padding:0;overflow:hidden;max-height:92vh;display:flex;flex-direction:column;border-radius:16px;background:var(--sf)">

    <!-- Öncelik renk çubuğu -->
    <div id="tk-pri-bar" style="height:4px;background:var(--ac);transition:background .25s;border-radius:16px 16px 0 0"></div>
    ...
    <div style="padding:16px 24px 12px;border-bottom:1px solid var(--b);display:flex;align-items:center;justify-content:space-between">
      <div>
        <div id="mo-tk-t" style="margin:0;font-size:16px;font-weight:700;color:var(--t)">Görev Ekle</div>
        <div id="tk-dept-workload-mini" style="font-size:11px;color:var(--t3);margin-top:2px"></div>
      </div>
      <button onclick="closeMo('mo-task')" style="background:none;border:none;cursor:pointer;font-size:18px;color:var(--t3);width:32px;height:32px;display:flex;align-items:center;justify-content:center;border-radius:8px;transition:background .15s" onmouseover="this.style.background='var(--s2)'" onmouseout="this.style.background='none'">✕</button>
    </div>
    ...
    <div style="flex:1;overflow-y:auto;padding:20px 24px;display:flex;flex-direction:column;gap:12px">
      ...
      <!-- SATIR 1 — Başlık -->
      <div>
        <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Görev Başlığı</label>
        <input class="fi" id="tk-title" placeholder="Görevi kısaca tanımlayın…" style="font-size:15px;font-weight:500;padding:10px 12px;border-radius:8px" oninput="updateTkPriBar();window._tkUpdateDeptWorkload?.()">
      </div>

      <!-- SATIR 2 — Öncelik + Personel + Durum -->
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Öncelik</label>
          <select class="fi" id="tk-pri" style="padding:10px 12px;border-radius:8px;font-size:13px" onchange="updateTkPriBar()">
        ...
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Personel</label>
          <select class="fi" id="tk-user" style="padding:10px 12px;border-radius:8px;font-size:13px" onchange="window._tkUpdateDeptWorkload?.();window.populatePusUsers?.()"></select>
          <div id="tk-atama-oneri" style="min-height:12px"></div>
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Durum</label>
          <select class="fi" id="tk-status" style="padding:10px 12px;border-radius:8px;font-size:13px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:10px">
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Başlangıç</label>
          <input type="date" class="fi" id="tk-start" style="padding:10px 12px;border-radius:8px;font-size:13px">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Bitiş</label>
          <input type="date" class="fi" id="tk-due" style="padding:10px 12px;border-radius:8px;font-size:13px">
        </div>
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Saat</label>
          <input type="time" class="fi" id="tk-due-time" style="padding:10px 12px;border-radius:8px;font-size:13px;width:110px">
        </div>
      </div>

      <!-- SATIR 4 — Job ID (serbest metin) -->
      <div>
        <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Job ID</label>
        <input class="fi" id="tk-jobid" placeholder="JOB-2026-XXXX (boş bırakılırsa otomatik atanır)" style="padding:10px 12px;border-radius:8px;font-size:13px;font-family:'DM Mono',monospace">
      </div>

      <!-- SATIR 5 — Departman -->
      <div>
        <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Departman</label>
        <select class="fi" id="tk-dept" style="padding:10px 12px;border-radius:8px;font-size:13px" onchange="window._tkUpdateDeptWorkload?.()">
          <option value="">— Seçiniz —</option>
        </select>
        <div id="tk-dept-workload" style="display:none;margin-top:6px;background:var(--s2);border-radius:8px;padding:8px 12px">
          ...
          <div style="height:5px;background:var(--b);border-radius:4px;overflow:hidden">
            <div id="tk-dept-wl-bar" style="height:100%;background:var(--ac);border-radius:4px;transition:width .4s;width:0%"></div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div style="border:1.5px solid var(--b);border-radius:8px;overflow:hidden">
          <div style="display:flex;gap:2px;padding:4px 8px;border-bottom:1px solid var(--b);background:var(--s2)">
            <button type="button" onclick="document.execCommand('bold')" style="background:none;border:none;cursor:pointer;font-weight:700;font-size:13px;padding:3px 8px;border-radius:4px;color:var(--t);font-family:inherit" title="Kalın">B</button>
            <button type="button" onclick="document.execCommand('italic')" style="background:none;border:none;cursor:pointer;font-style:italic;font-size:13px;padding:3px 8px;border-radius:4px;color:var(--t);font-family:inherit" title="İtalik">I</button>
            <button type="button" onclick="document.execCommand('underline')" style="background:none;border:none;cursor:pointer;text-decoration:underline;font-size:13px;padding:3px 8px;border-radius:4px;color:var(--t);font-family:inherit" title="Altı Çizili">U</button>
          ...
          <div contenteditable="true" id="tk-desc-rich" style="min-height:80px;max-height:160px;overflow-y:auto;padding:10px 12px;font-size:13px;color:var(--t);outline:none;font-family:inherit;line-height:1.6" data-placeholder="Detay, bağlam, gereksinimler..."></div>
        </div>
      ...
      <!-- Maliyet -->
      <div>
        <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">İşlem Tutarı <span style="font-weight:400;color:var(--t3)">(opsiyonel)</span></label>
        <div style="position:relative">
          <span style="position:absolute;left:12px;top:50%;transform:translateY(-50%);font-size:13px;color:var(--t3)">₺</span>
          <input class="fi" type="number" id="tk-cost" placeholder="0.00" style="padding:10px 12px 10px 28px;border-radius:8px;font-size:13px">
        </div>
      </div>

      <!-- Alt Görevler (accordion) -->
      <div style="border:1px solid var(--b);border-radius:8px;overflow:hidden">
        <div style="padding:10px 14px;background:var(--s2);display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="window._tkToggleSubtasks?.()">
          <span style="font-size:12px;font-weight:600;color:var(--t2)">Alt Görevler</span>
        ...
        <div id="tk-subtasks-wrap" style="padding:12px 14px;display:block">
          ...
          <button type="button" onclick="window._tkAddSubtask?.()" style="margin-top:6px;background:none;border:1px dashed var(--b);border-radius:8px;width:100%;padding:8px;font-size:12px;color:var(--t3);cursor:pointer;font-family:inherit;transition:all .15s" onmouseover="this.style.borderColor='var(--ac)';this.style.color='var(--ac)'" onmouseout="this.style.borderColor='var(--b)';this.style.color='var(--t3)'">+ Alt Görev Ekle</button>
        </div>
      </div>
      ...
      <div style="border:1px solid var(--b);border-radius:8px;overflow:hidden">
        <div style="padding:10px 14px;background:var(--s2);display:flex;align-items:center;justify-content:space-between;cursor:pointer" onclick="window._tkToggleAdvanced?.()">
          <span style="font-size:12px;font-weight:600;color:var(--t2)">Gelişmiş Seçenekler</span>
          <span id="tk-adv-arrow" style="font-size:12px;color:var(--t3);transition:transform .2s">▶</span>
        </div>
        <div id="tk-advanced-wrap" style="display:none;padding:14px;flex-direction:column;gap:10px">
          <div>
            <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Etiketler <span style="font-weight:400;color:var(--t3)">(virgülle ayır)</span></label>
            <input class="fi" id="tk-tags" placeholder="satınalma, acil, operasyon…" style="padding:10px 12px;border-radius:8px;font-size:13px">
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <div>
              <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Referans Link</label>
              <input class="fi" id="tk-link" placeholder="https://…" style="padding:10px 12px;border-radius:8px;font-size:13px">
            </div>
            <div>
              <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Süre (dk)</label>
              <input class="fi" type="number" id="tk-duration" placeholder="60" style="padding:10px 12px;border-radius:8px;font-size:13px">
            </div>
          </div>
          <div>
            <label style="display:block;font-size:12px;font-weight:500;color:var(--t2);margin-bottom:4px">Dosya Eki</label>
            <input type="file" class="fi" id="tk-file" accept=".pdf,.jpg,.png,.docx,.xlsx" style="font-size:13px;padding:10px 12px;border-radius:8px" onchange="if(g('tk-fp'))g('tk-fp').textContent=this.files[0]?'📎 '+this.files[0].name:''">
            <div id="tk-fp" style="font-size:11px;color:var(--ac);margin-top:4px;min-height:14px"></div>
          </div>
          <div>
            <label style="display:block;font-size:12px;font-weight:600;color:var(--t2);margin-bottom:8px">Katılımcılar & İzleyiciler</label>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
              <div>
                <div style="font-size:11px;font-weight:500;color:var(--ac);margin-bottom:6px">Katılımcılar</div>
                <div id="tk-participants-list" style="display:flex;flex-direction:column;gap:4px;max-height:120px;overflow-y:auto"></div>
              </div>
              <div>
                <div style="font-size:11px;font-weight:500;color:#8B5CF6;margin-bottom:6px">İzleyiciler</div>
    ...
    <div style="padding:14px 24px;border-top:1px solid var(--b);display:flex;justify-content:space-between;align-items:center;background:var(--s2)">
      <button class="btn btns" onclick="window.openTaskTemplates?.()" style="font-size:13px;padding:8px 16px;border-radius:8px">Şablon</button>
      <div style="display:flex;gap:8px">
        <button class="btn btns" onclick="closeMo('mo-task')" style="font-size:13px;padding:10px 20px;border-radius:8px">İptal</button>
        <button class="btn btnp" onclick="saveTask()" style="padding:10px 28px;border-radius:8px;font-size:14px;font-weight:700">Kaydet</button>
      </div>
    </div>
