  ...
  function _naRenderSummary(m) {
    ...
    var card = function(label, value, sub, color, onclickAction) {
      ...
      var clickAttrs = clickable
        ...
        : '';
      /* NAKIT-KPI-FLEX-FIX-001: font-weight 800 + line-height 1.2 + sub margin-top 6px */
      return '<div style="background:var(--sf,#fff);border:1px solid var(--b,#e5e7eb);border-radius:12px;padding:16px' + clickStyle + '"' + clickAttrs + '>'
        + '<div style="font-size:9px;font-weight:700;color:var(--t3,#6b7280);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;display:block">' + _naEsc(label) + '</div>'
        + '<div style="font-size:22px;font-weight:800;color:' + color + ';font-family:monospace;display:block;line-height:1.2">' + _naEsc(value) + '</div>'
        + '<div style="font-size:10px;color:var(--t3,#6b7280);margin-top:6px;display:block">' + _naEsc(sub) + '</div>'
      + '</div>';
    };
