...
function openUrunForm(editId) {
  ...
  mo.innerHTML = '<div style="background:var(--sf);border-radius:12px;border:0.5px solid var(--b);width:700px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden">'
    ...
    + '<div style="border:0.5px solid #A32D2D;border-radius:6px;padding:10px 14px;margin-bottom:8px;background:#FCEBEB">'
    + '<div style="font-size:10px;font-weight:500;color:#A32D2D;margin-bottom:6px">⚠ Bu görsel müşteriye sunulacak teklifte kullanılacaktır — profesyonel olması zorunludur</div>'
    + '<div style="font-size:9px;color:#791F1F;line-height:1.6">'
    + '<div style="margin-bottom:4px;font-weight:500">✓ Kabul edilen görsel türleri:</div>'
    + '<div>1. Beyaz/düz arka plan — ürün ortalanmış, gölgesiz</div>'
    + '<div>2. Stüdyo çekimi — profesyonel ışık, temiz arka plan</div>'
    + '<div>3. Teknik çizim — ölçülü, vektörel</div>'
    + '<div>4. Ambalaj görseli — kutu/paket, marka logosu görünür</div>'
    + '<div>5. Gerçek kullanım — ürün yerinde, bağlam anlaşılır</div>'
    + '<div style="margin-top:6px;font-weight:500">✗ Kabul edilmeyen: Bulanık · Filigran · Karmaşık arka plan · Ekran fotoğrafı</div>'
    + '</div></div>'
    ...
    + '<div style="font-size:9px;color:var(--t3);font-weight:500;margin-bottom:6px">DATASHEET <span style="font-size:8px;color:#854F0B">(Teknik ürünlerde zorunlu)</span></div>'
    + '<label style="display:block;cursor:pointer"><div style="border:0.5px dashed var(--b);border-radius:4px;padding:8px;text-align:center;font-size:10px;color:var(--t3)">+ PDF Yükle</div>'
    + '<input type="file" accept=".pdf" id="uf2-datasheetFile" onchange="document.getElementById(\'uf2-datasheetAd\').textContent=this.files[0]?.name||\'\';" style="display:none"></label>'
    + '<div id="uf2-datasheetAd" style="font-size:9px;color:var(--t3);margin-top:4px">' + _esc(u.datasheetAd || '') + '</div>'
    + _fi('datasheetTarih', 'ÜRETİM TARİHİ', u.datasheetTarih || '', false, 'month', '')
    + '</div>'
    + '<div style="border:0.5px solid var(--b);border-radius:6px;padding:10px">'
    + '<div style="font-size:9px;color:var(--t3);font-weight:500;margin-bottom:6px">ÜRÜN KATALOĞU <span style="font-size:8px;color:#854F0B">(Varsa zorunlu)</span></div>'
    + '<label style="display:block;cursor:pointer"><div style="border:0.5px dashed var(--b);border-radius:4px;padding:8px;text-align:center;font-size:10px;color:var(--t3)">+ PDF/Görsel Yükle</div>'
    + '<input type="file" accept=".pdf,.jpg,.png" id="uf2-katalogFile" onchange="document.getElementById(\'uf2-katalogAd\').textContent=this.files[0]?.name||\'\';" style="display:none"></label>'
    + '<div id="uf2-katalogAd" style="font-size:9px;color:var(--t3);margin-top:4px">' + _esc(u.katalogAd || '') + '</div>'
    + _fi('katalogYil', 'BAKIM YILI', u.katalogYil || '', false, 'number', 'Örn: 2024')
    + '</div>'
    + '</div>'
    + '<div style="border:0.5px solid var(--b);border-radius:6px;padding:10px;margin-top:10px">'
    + '<div style="font-size:9px;color:var(--t3);font-weight:500;margin-bottom:6px">DİĞER TEKNİK DÖKÜMANLAR</div>'
    + '<label style="display:block;cursor:pointer"><div style="border:0.5px dashed var(--b);border-radius:4px;padding:8px;text-align:center;font-size:10px;color:var(--t3)">+ Dosya Ekle</div>'
    + '<input type="file" accept=".pdf,.doc,.docx,.xlsx" multiple id="uf2-dokumanlar" onchange="var n=this.files.length;document.getElementById(\'uf2-dokumanAd\').textContent=n+\' dosya seçildi\';" style="display:none"></label>'
    + '<div id="uf2-dokumanAd" style="font-size:9px;color:var(--t3);margin-top:4px"></div>'
    + '</div>'
    + '</div>'
...
function _renderForm() {
  try {
    ...
    const SAG = '<div style="flex:1;overflow-y:auto;padding:18px 22px">'
      ...
      +'<button type="button" onclick="event.stopPropagation();document.execCommand(\'underline\')" style="font-size:11px;text-decoration:underline;padding:2px 7px;border:0.5px solid var(--b);border-radius:3px;background:transparent;cursor:pointer;color:var(--t)">U</button>'
      ...
      +'</div>'
      +'<div id="uf2-teknikAciklama-div" contenteditable="true" onclick="event.stopPropagation()" onkeydown="event.stopPropagation()" style="min-height:80px;max-height:200px;overflow-y:auto;padding:8px 10px;font-size:12px;color:var(--t);font-family:inherit;outline:none;line-height:1.5">'+(u.teknikAciklama||u.aciklama||'')+'</div>'
      +'<input type="hidden" id="uf2-teknikAciklama">'
      +'</div>'
