...
function renderResmi() {
  ...
  items.sort((a,b)=>(b.tarih||'').localeCompare(a.tarih||'')).forEach(r=>{
    ...
    tr.innerHTML=`
      ...
      <td><div style="font-weight:500;font-size:13px">${window._esc(r.konu||'—')}</div>${r.not?`<div style="font-size:10px;color:var(--t3)">${window._esc(r.not.slice(0,60))}</div>`:''}</td>
      <td style="font-size:12px;color:var(--t2)">${window._esc(r.taraf||'—')}</td>
