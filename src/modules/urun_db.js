  ...
  mo.innerHTML = '<div class="moc" style="max-width:800px;padding:0;border-radius:14px;overflow:hidden;max-height:94vh;display:flex;flex-direction:column">'
    ...
    + '<div style="flex:1;overflow-y:auto;padding:18px 20px;display:flex;flex-direction:column;gap:10px">'
      ...
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">'
        ...
        + '<div><div class="fl">SATICI SINIFI</div><input class="fi" id="ud-vendorClass" value="' + esc(u?.vendorClass || '') + '" placeholder="A/B/C"></div>'
        + '<div><div class="fl">SATICI ÜRÜN KODU *</div><input class="fi" id="ud-vendorCode" value="' + esc(u?.vendorCode || '') + '" placeholder="ABC-123"></div>'
      + '</div>'
      // Satır 2: Duay Kodu + Duay Adı + Orijinal Adı
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">'
        + '<div><div class="fl">DUAY ÜRÜN KODU</div><input class="fi" id="ud-duayCode" value="' + esc(u?.duayCode || '') + '" readonly style="background:var(--s2);color:var(--t3)" placeholder="Otomatik"></div>'
        + '<div><div class="fl">DUAY ÜRÜN ADI *</div><input class="fi" id="ud-duayName" value="' + esc(u?.duayName || '') + '" placeholder="Ürün adı"></div>'
        + '<div><div class="fl">ORİJİNAL ÜRN ADI</div><input class="fi" id="ud-origName" value="' + esc(u?.origName || '') + '"></div>'
      + '</div>'
      // Satır 3: Standart Ad + Fatura Adı + Teknik Açıklama
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">'
        + '<div><div class="fl">STANDART AD (CI/PL/BL)</div><input class="fi" id="ud-stdName" value="' + esc(u?.stdName || '') + '"></div>'
        + '<div><div class="fl">FATURA ÜRÜN ADI</div><input class="fi" id="ud-invoiceName" value="' + esc(u?.invoiceName || '') + '"></div>'
        + '<div><div class="fl">GÜMRÜKÇÜ TANIMI</div><input class="fi" id="ud-customsDesc" value="' + esc(u?.customsDesc || '') + '"></div>'
      + '</div>'
      // Satır 4: Kategori + Marka + Renk + Menşei
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px">'
        + '<div><div class="fl">KATEGORİ</div><input class="fi" id="ud-category" value="' + esc(u?.category || '') + '"></div>'
        + '<div><div class="fl">MARKA</div><input class="fi" id="ud-brand" value="' + esc(u?.brand || '') + '"></div>'
        + '<div><div class="fl">RENK</div><input class="fi" id="ud-color" value="' + esc(u?.color || '') + '"></div>'
      ...
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:10px">'
        + '<div><div class="fl">GTİP</div><input class="fi" id="ud-gtip" value="' + esc(u?.gtip || '') + '" placeholder="8501.10.00"></div>'
        + '<div><div class="fl">VERGİ %</div><input class="fi" type="number" id="ud-taxRate" value="' + esc(u?.taxRate || '') + '"></div>'
        + '<div><div class="fl">BİRİM</div><input class="fi" id="ud-unit" value="' + esc(u?.unit || 'Adet') + '"></div>'
      ...
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:10px">'
        + '<div><div class="fl">NET AĞIRLIK (kg)</div><input class="fi" type="number" id="ud-netW" value="' + esc(u?.netWeight || '') + '"></div>'
        + '<div><div class="fl">BRÜT AĞIRLIK (kg)</div><input class="fi" type="number" id="ud-grossW" value="' + esc(u?.grossWeight || '') + '"></div>'
        + '<div><div class="fl">EN (cm)</div><input class="fi" type="number" id="ud-pkgW" value="' + esc(u?.pkgWidth || '') + '"></div>'
        + '<div><div class="fl">BOY (cm)</div><input class="fi" type="number" id="ud-pkgL" value="' + esc(u?.pkgLength || '') + '"></div>'
        + '<div><div class="fl">YÜKSEKLİK (cm)</div><input class="fi" type="number" id="ud-pkgH" value="' + esc(u?.pkgHeight || '') + '"></div>'
      + '</div>'
      // Teknik açıklama + Not
