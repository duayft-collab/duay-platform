...
/* URUN-FORMU-PROGRESS-BAR-FIX-001: Batch ürün modal (urun_db.js openUrunModal) için per-row progress.
   Eski ur-* prefix'li versiyon dead code'du — _openUrunModal asla gösterilmiyordu (urun_db.js:889 override). */
(function _urunBatchProgressInit() {
  var URUN_FIELDS_PREFIX = ['udb-duayName-','udb-origName-','udb-category-','udb-vendor-','udb-vendorCode-','udb-origin-','udb-unit-','udb-netW-','udb-grossW-','udb-marka-','udb-raf-'];
  var ZORUNLU_PREFIX = ['udb-duayName-','udb-vendor-','udb-category-'];
  var TOTAL = URUN_FIELDS_PREFIX.length;

  function _rowProgress(idx) {
    var dolu = 0;
    URUN_FIELDS_PREFIX.forEach(function(p){
      var el = document.getElementById(p + idx);
      if (el && String(el.value || '').trim()) dolu++;
    });
    var eksikZ = ZORUNLU_PREFIX.filter(function(p){
      var el = document.getElementById(p + idx);
      return !el || !String(el.value || '').trim();
    }).length;
    return { dolu: dolu, pct: Math.round(dolu/TOTAL*100), eksikZ: eksikZ };
  }

  function _ensureProgressSpan(rowEl) {
    var span = rowEl.querySelector('[data-urun-progress]');
    if (span) return span;
    /* "📦 Ürün N" label'ını bul — font-size:13px + font-weight:600 */
    var header = rowEl.querySelector('span[style*="font-size:13px"]');
    if (!header) return null;
    span = document.createElement('span');
    span.setAttribute('data-urun-progress', '1');
    span.style.cssText = 'margin-left:12px;font-size:10px;font-variant-numeric:tabular-nums;font-weight:500';
    header.parentNode.insertBefore(span, header.nextSibling);
    return span;
  }

  window._urunFormUpdateAll = function() {
    var modal = document.getElementById('mo-urun-db');
    if (!modal) return;
    var rows = modal.querySelectorAll('.udb-card[id^="udb-row-"]');
    var hepsiOK = rows.length > 0;
    rows.forEach(function(rowEl) {
      var m = rowEl.id.match(/udb-row-(\d+)/);
      if (!m) return;
      var idx = m[1];
      var r = _rowProgress(idx);
      if (r.eksikZ > 0) hepsiOK = false;
      var span = _ensureProgressSpan(rowEl);
      if (!span) return;
      span.textContent = r.dolu + '/' + TOTAL + ' alan · %' + r.pct + (r.eksikZ > 0 ? ' — ' + r.eksikZ + ' zorunlu eksik' : '');
      span.style.color = r.pct < 40 ? '#E0574F' : r.pct < 70 ? '#B4730F' : '#1A8D6F';
    });
    /* "Tümünü Kaydet" butonu — tüm satırlarda zorunlu dolu mu? */
    var btn = null;
    var btns = modal.querySelectorAll('button');
    for (var i = 0; i < btns.length; i++) {
      if (/Tümünü Kaydet/i.test(btns[i].textContent || '')) { btn = btns[i]; break; }
    }
    if (btn) {
      btn.disabled = !hepsiOK;
      btn.style.opacity = hepsiOK ? '1' : '0.5';
      btn.style.cursor = hepsiOK ? 'pointer' : 'not-allowed';
      btn.title = hepsiOK ? '' : 'Her üründe zorunlu alanlar (Ürün Adı TR · Tedarikçi · Kategori) dolu olmalı';
    }
  };

  /* Event delegation — sadece udb-* input'larında */
  document.addEventListener('input', function(e) {
    if (e.target && e.target.id && e.target.id.indexOf('udb-') === 0) window._urunFormUpdateAll();
  });
  document.addEventListener('change', function(e) {
    if (e.target && e.target.id && e.target.id.indexOf('udb-') === 0) window._urunFormUpdateAll();
  });

  /* MutationObserver — modal açılışı + yeni satır ekleme */
  try {
    new MutationObserver(function(mutations) {
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var j = 0; j < added.length; j++) {
          ...
          if (n.id === 'mo-urun-db' || (n.classList && n.classList.contains('udb-card')) || (n.querySelector && n.querySelector('.udb-card'))) {
            setTimeout(window._urunFormUpdateAll, 50);
            return;
          }
        }
      }
...
window.renderSatisTeklifleri = function() {
  ...
  if (statsEl) statsEl.innerHTML = '<div style="padding:14px 20px;border-right:0.5px solid var(--b)"><div style="font-size:10px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">Toplam</div><div style="font-size:22px;font-weight:600;font-variant-numeric:tabular-nums">' + d.length + '</div></div>'
    + '<div style="padding:14px 20px;border-right:0.5px solid var(--b)"><div style="font-size:10px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">Onay Bekleyen</div><div style="font-size:22px;font-weight:600;color:#D97706;font-variant-numeric:tabular-nums">' + d.filter(function(t){return t.durum==='onay';}).length + '</div></div>'
    + '<div style="padding:14px 20px;border-right:0.5px solid var(--b)"><div style="font-size:10px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">Kabul Edilen</div><div style="font-size:22px;font-weight:600;color:#16A34A;font-variant-numeric:tabular-nums">' + d.filter(function(t){return t.durum==='kabul';}).length + '</div></div>'
    + '<div style="padding:14px 20px;border-right:0.5px solid var(--b)"><div style="font-size:10px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">Toplam Değer</div><div style="font-size:22px;font-weight:600;color:var(--ac);font-variant-numeric:tabular-nums">$' + Math.round(_stToplamDeger).toLocaleString('tr-TR') + '</div></div>'
    + '<div style="padding:14px 20px"><div style="font-size:10px;color:var(--t3);text-transform:uppercase;margin-bottom:4px">Ort. Marj</div><div style="font-size:22px;font-weight:600;color:' + _stMarjColor + ';font-variant-numeric:tabular-nums">%' + String(_stOrtMarj.toFixed(1)).replace('.', ',') + '</div></div>';
...
window._printSatisTeklif = function(id) {
  ...
  var banka = bankaList[0] || { banka:'Albaraka Türk', sube:'Alibeyköy/117', iban:'', swift:'BTFHTRIS', hesapSahibi:(window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || (window.PI_ADRES?.unvanTR) || 'Duay Uluslararası Ticaret Ltd. Şti.' };
...
(function _ensureDemoBanka() {
  ...
  bankalar.unshift({
    ...
    hesapSahibi: (window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || (window.PI_ADRES?.unvanTR) || 'Duay Uluslararası Ticaret Ltd. Şti.',
    createdAt: new Date().toISOString()
  });
...
window.renderEvrakPaketi = function() {
  ...
  h += '</div></div>';
  h += '<div style="margin-top:16px;padding:8px 10px;background:#FAEEDA;border-radius:6px;font-size:9px;color:#633806">Haz\u0131rlayan: <strong>' + (window.CU?.()?.displayName || '\u2014') + '</strong> \u00b7 Bu belge ' + ((window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || 'Duay Uluslararası Ticaret Ltd. Şti.') + ' i\u00e7 kullan\u0131m\u0131na mahsustur.</div>';
  h += '</div>';
  panel.innerHTML = h;
};
...
window._epPDFFooter = function() {
  return '<div style="margin-top:30px;padding-top:8px;border-top:0.5px solid #ddd;font-size:7px;color:#999;display:flex;justify-content:space-between"><span>' + ((window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || 'Duay Uluslararası Ticaret Ltd. Şti.') + ' \u00b7 ' + ((window.SIRKET_DATA && window.SIRKET_DATA.adres_tr) || 'Karadolap Mh. Neşeli Sk. 1/5 Eyüpsultan İstanbul') + '</span><span>Tel: ' + ((window.SIRKET_DATA && window.SIRKET_DATA.tel) || '+90 212 625 5 444') + '</span></div><div style="text-align:center;font-size:6px;color:#bbb;font-style:italic;margin-top:4px">Bu belge ' + ((window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || 'Duay Uluslararası Ticaret Ltd. Şti.') + ' i\u00e7 kullan\u0131m\u0131na mahsustur.</div>';
};
...
window._epKapakPDF = function() {
  ...
  h += '<h1>' + ((window.SIRKET_DATA && window.SIRKET_DATA.hesapSahibi) || 'Duay Uluslararası Ticaret Ltd. Şti.') + '</h1><h2>MUHASEBE DOSYASI KAPAK SAYFASI</h2>';
