...
function renderIkZimmet(){
  ...
  if(uSel&&uSel.options.length<=1)
    uSel.innerHTML='<option value="0">Tüm Personel</option>'+users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');
...
function openIzinModal(id){
  ...
  if(sel){
    if(_isAdmin()){sel.innerHTML=users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');sel.disabled=false;}
...
function renderPuantaj(){
  ...
  if(fu){
    if(_isAdmin()){fu.innerHTML='<option value="">Tüm Personel</option>'+users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');fu.style.display='inline-block';}
    else if(hasPuantajYetki()){const allowed=[cu,...getPuantajSorumluIds().map(uid=>users.find(x=>x.id===uid)).filter(Boolean)];fu.innerHTML='<option value="">Sorumlu Personelim</option>'+allowed.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');fu.style.display='inline-block';}
    else fu.style.display='none';
  }
  const mu=_gi('mp-user');
  if(mu){
    if(_isAdmin())mu.innerHTML=users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');
    else if(hasPuantajYetki()){const allowed=[cu,...getPuantajSorumluIds().map(uid=>users.find(x=>x.id===uid)).filter(Boolean)];mu.innerHTML=allowed.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');}
    else mu.innerHTML=`<option value="${cu.id}">${window._esc(cu.name)}</option>`;
...
function renderPuantajSummary(data,users){
  ...
  rows.forEach((r,i)=>{
    ...
    tr.innerHTML=`<td style="padding:11px 14px"><div style="display:flex;align-items:center;gap:8px"><div style="width:30px;height:30px;border-radius:9px;background:${avBg};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;flex-shrink:0">${_initials(u.name)}</div><div><div style="font-size:13px;font-weight:600;color:var(--t)">${window._esc(u.name)}</div><div style="display:flex;align-items:center;gap:4px;margin-top:2px"><div style="height:4px;width:48px;background:var(--b);border-radius:2px;overflow:hidden"><div style="height:100%;background:${attendRate>=90?'#22C55E':attendRate>=70?'#F59E0B':'#EF4444'};width:${attendRate}%"></div></div><span style="font-size:10px;color:var(--t3)">%${attendRate} devam</span></div></div></div></td>
...
function openPuanModal(id){
  const users=loadUsers();
  const mu=_gi('mp-user');if(mu)mu.innerHTML=users.map(u=>`<option value="${u.id}">${window._esc(u.name)}</option>`).join('');
  if(id){
