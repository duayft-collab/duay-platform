...
/* CACHE-BUMP-2026-05-06-V193 V193 MUHASEBAT-NAKIT-AKISI-MANUEL-WORKS paketi (4 EDIT): cash_flow_works.js (Firestore CRUD + migration + dirty-state + audit) + cash_flow.js refactor (kaynak/banka alanı + datalist + badge + toolbar entegrasyonu + event delegation) + cash_flow_works_ui.js (kayıtlı çalışmalar modal + Aktif/Arşiv tab + kart + yeni/yeniden adlandır mini modal) + cash_flow_excel.js (XLSX export, UTF-8 Türkçe). V191'den (lojistik paketi) sonra cache bump — tüm V193 dosyaları kullanıcı tarayıcılarına otomatik düşsün diye. T2-prev: V193 paket sonu */
const CACHE_NAME    = 'duay-platform-v193';
const CACHE_VERSION = '193.0.0';
