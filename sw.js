...
/* CACHE-BUMP-2026-05-06-V193e6 V193 paketi EDIT 4-5-6 (Genişletilmiş kapsam): cash_flow_compute.js (YENİ, pure compute layer — TRY hesap + running balance + filter helpers) + cash_flow_edit.js (YENİ, satır düzeltme modal + gerçek update) + cash_flow.js refactor (filter state + açılış kasa bakiyesi + 10-kolon tablo + sıra no + döviz/kur ikinci satır + TRY karşılığı + kasa mevcudu + Düzelt+Sil aksiyon) + cash_flow_excel.js REWRITE (12 kolon, filtre uygulanmış liste, freeze header, sabit kolon düzeni) + cash_flow_pdf.js (YENİ, html2pdf off-screen render, A4 landscape, sade kurumsal, zebra row). V193 → V193e6 cache bump tüm 7 cash_flow* dosyasını invalidate eder. PATCH 1 cache key (V193p1) artık geçerli değil. T2-prev: V193 paket EDIT 6 final */
const CACHE_NAME    = 'duay-platform-v193e6';
const CACHE_VERSION = '193.6.0';
